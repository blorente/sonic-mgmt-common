...
class ContainerToListPlugin:
    ...
    def process_children(self, stmt, level):
        if level == 3 and stmt.keyword == "container":
            # Level 0 would be all top-level statements.
            # Level 1 && container would be the top-level data definition group (e.g. container sonic-acl).
            # Level 2 && container would be a table name container (e.g. container ACL_TABLE).
            # level 3 && container indicates the table is a singleton w/ key equal the container's name.
...
class Utils:
    ...
    def use_sonic_extension(stmt, ext, key_name):
        ...
        if sonic_ext_prefix is None:
            ...
            # Insert the import statement after the yang-version, namespace, and prefix but before other data
            # such as containers, revision, etc.
            insert_index = len(module_stmt.substmts)
            for i, ss in enumerate(module_stmt.substmts):
                if ss.keyword in ['import', 'organization', 'contact', 'description', 'revision', 'container']:
                    insert_index = i
                    break
            module_stmt.substmts.insert(insert_index, import_stmt)
