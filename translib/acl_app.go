...
func (app *AclApp) initialize(data appData) {
	log.V(lvl.INFO).Info("initialize:acl:path =", data.path)
...
func (app *AclApp) translateCRUCommon(d *db.DB, opcode int) ([]db.WatchKeys, error) {
	...
	log.V(lvl.INFO).Info("translateCRUCommon:acl:path =", app.pathInfo.Template)
...
func (app *AclApp) processCommon(d *db.DB, opcode int) error {
	...
	log.V(lvl.INFO).Infof("processCommon--Path Received: %s", app.pathInfo.Template)
	...
	if !util.IsValueScalar(reflect.ValueOf(*app.ygotTarget)) && util.IsValuePtr(reflect.ValueOf(*app.ygotTarget)) {
		log.V(lvl.INFO).Infof("processCommon: Target object is a <%s> of Type: %s", targetType.Kind().String(), targetType.Elem().Name())
...
func (app *AclApp) convertInternalToOCAcl(aclName string, aclSets *ocbinds.OpenconfigAcl_Acl_AclSets, aclSet *ocbinds.OpenconfigAcl_Acl_AclSets_AclSet) {
	if len(aclName) > 0 {
		...
		for acln := range app.aclTableMap {
			...
			/* Google: Adding this guard to prevent openconfig-acl's from returning
			 * junk keys (key is name and acl-type) since the acl app may find entries
			 * in the ConfigDB ACL_TABLE for things like our link-qual which do not
			 * have a "type" field in the DB entry. */
