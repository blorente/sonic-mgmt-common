...
func generateGetResponse(targetUri string, root *ygot.GoStruct, fmtType TranslibFmtType) (GetResponse, error) {
	...
	// Creates a metadata reference to the original node.
	if m := utils.FindYGRootMeta(*root); m != nil {
		m.CreateReference(parentCloneObj.(ygot.GoStruct), parentNode.(ygot.GoStruct))
	}
