...
func (binder *requestBinder) validateObjectType(errObj error) error {
	...
	if binder.opcode == GET || binder.isSonicModel {
		...
		if len(tmpStr) > 0 {
			log.V(lvl.INFO).Info("validateObjectType ==> GET == return err string ==> ", tmpStr)
		...
		if strings.Contains(errStr, "ERROR_READONLY_OBJECT_FOUND") {
			log.V(lvl.INFO).Info("validateObjectType ==> WRITE == return err string")
...
func (binder *requestBinder) validateRequest(deviceObj *ocbinds.Device) error {
	...
	if binder.isSonicModel {
		log.V(lvl.WARNING).Info("Translib: RequestBinder: Skipping the vaidatiion of the given sonic yang model request..")
		return nil
	}
	...
	case UPDATE, REPLACE:
		...
		if ok {
			if !binder.isSonicModel {
				...
				log.V(lvl.WARNING).Info("Translib: Request binder: Valdation skipping for sonic yang model..")
			}
		}
...
func (binder *requestBinder) unMarshallUri(deviceObj *ocbinds.Device) (*interface{}, error) {
	...
	case UPDATE, REPLACE:
		...
		log.V(lvl.DEBUG).Info("requestBinder: modified path is: ", gpath)
	...
	if errYg != nil {
		log.V(lvl.DEBUG).Info("Error in creating the target object: ", errYg)
		return nil, errYg
	} else {
