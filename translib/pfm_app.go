...
func init() {
	// log.V(lvl.INFO).Info("Init called for Platform module")
...
func (app *PlatformApp) initialize(data appData) {
	log.V(lvl.INFO).Info("initialize:if:path =", data.path)
...
func (app *PlatformApp) translateGet(dbs [db.MaxDB]*db.DB) error {
	var err error
	log.V(lvl.INFO).Info("PlatformApp: translateGet - path: ", app.path.Path)
	return err
}
...
func (app *PlatformApp) processReplace(d *db.DB) (SetResponse, error) {
	var err error
	var resp SetResponse
	log.V(lvl.INFO).Info("processReplace:intf:path =", app.path)
...
func (app *PlatformApp) processGet(dbs [db.MaxDB]*db.DB, fmtType TranslibFmtType) (GetResponse, error) {
	pathInfo := app.path
	log.V(lvl.INFO).Infof("Received GET for PlatformApp Template: %s ,path: %s, vars: %v",
	...
	if perr != nil {
		log.V(lvl.INFO).Infof("getYangPathFromUri failed.")
...
func (app *PlatformApp) getEepromDbObj() EepromDb {
	log.V(lvl.INFO).Infof("parseEepromDb Enter")
...
func (app *PlatformApp) getSysEepromFromDb(eeprom *ocbinds.OpenconfigPlatform_Components_Component_State, all bool) error {
	...
	log.V(lvl.INFO).Infof("getSysEepromFromDb Enter")
...
func (app *PlatformApp) doGetSysEeprom() error {
	...
	log.V(lvl.INFO).Infof("Preparing collection for system eeprom")
