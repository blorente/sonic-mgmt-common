...
func TestUnMarshallUri(t *testing.T) {
	...
	tests := []struct {
		...
		payload:     []byte{},
		appRootType: reflect.TypeOf(ocbinds.OpenconfigAcl_Acl{}),
		want:        "ACL_IPVX is not a valid value for enum field Type, type ocbinds.E_OpenconfigAcl_ACL_TYPE",
	}, {
		//Negative test case
		tid:         9,
	...
	for _, tt := range tests {
		...
		if err != nil {
			// Negative test case
			if strings.Contains(err.Error(), tt.want) == false {
				t.Error("Error in unmarshalling the URI: didn't get the expected error, want:", tt.want, "got:", err)
			}
		} else {
