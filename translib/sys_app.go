...
func init() {
	// log.V(lvl.INFO).Info("SysApp: Init called for System module")
...
func (app *SysApp) initialize(data appData) {
	log.V(lvl.INFO).Info("SysApp: initialize:if:path =", data.path)
...
func (app *SysApp) translateGet(dbs [db.MaxDB]*db.DB) error {
	var err error
	log.V(lvl.INFO).Info("SysApp: translateGet:intf:path =", app.path)
	return err
}
...
func (app *SysApp) getSystemProcess(sysproc *ocbinds.OpenconfigSystem_System_Processes_Process, pid uint64) {
	...
	log.V(lvl.INFO).Info("getSystemProcess pid=", pid)
...
func (app *SysApp) getSystemProcesses(sysprocs *ocbinds.OpenconfigSystem_System_Processes, ispid bool) {
	log.V(lvl.INFO).Infof("getSystemProcesses Entry")
	...
	if ispid == true {
		...
		for pid, _ := range app.procTable {
			...
			if err != nil {
				log.V(lvl.INFO).Infof("sysprocs.NewProcess failed")
				return
			}
...
func (app *SysApp) processGet(dbs [db.MaxDB]*db.DB, fmtType TranslibFmtType) (GetResponse, error) {
	log.V(lvl.INFO).Info("SysApp: processGet Path: ", app.path.Path)
	...
	log.V(lvl.INFO).Info("targetUriPath : ", targetUriPath, "Args: ", app.path.Vars)
