...
func register(path string, info *appInfo) error {
	var err error
	log.V(lvl.INFO).Info("Registering for path =", path)
...
// Unregister the app module for the given path. This is used for testing.
...
func getAppModuleInfo(path string) (*appInfo, error) {
	log.V(lvl.DEBUG).Info("getAppModule called for path =", path)
	...
	for pattern, app := range appMap {
		...
		log.V(lvl.DEBUG).Info("found the entry in the map for path =", pattern)

		return app, nil
	}

	/* If no specific app registered fallback to default/common app */
	log.V(lvl.DEBUG).Infof("No app module registered for path %s hence fallback to default/common app", path)
	app := getCommonAppInfo()

	return app, nil
}

func getCommonAppInfo() *appInfo {
	return appMap["*"]
}
...
func getAppInterface(appType reflect.Type) (appInterface, error) {
	...
	if !ok {
		...
		log.V(lvl.DEBUG).Info("cast to appInterface worked", app)
	}
