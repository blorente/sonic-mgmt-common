...
func setEmitJSONImpl() {
	...
	if _, err := os.Stat(markerFile); err == nil {
		glog.V(lvl.INFO).Infof("Marker file %s exists; will use ygot.EmitJSON()", markerFile)
		atomic.StoreInt32(&useYgotEmitJSON, 1)
	} else {
		glog.V(lvl.INFO).Infof("Marker file %s not found (err = %v). Will use internal EmitJSON", markerFile, err)
...
func ygotEmitJSON(s ygot.ValidatedGoStruct, opts *EmitJSONOptions) ([]byte, error) {
	glog.V(lvl.INFO).Infof("Render %T using ygot.EmitJSON", s)
