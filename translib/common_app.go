...
func init() {
	...
	if mdlCpblt == nil {
		log.V(lvl.WARNING).Info("Failure in fetching model capabilities data.")
	} else {
		for yngMdlNm, mdlDt := range mdlCpblt {
			err := addModel(&ModelData{Name: yngMdlNm, Org: mdlDt.Org, Ver: mdlDt.Ver})
			if err != nil {
				log.V(lvl.WARNING).Infof("Adding model data for module %v to appinterface failed with error=%v", yngMdlNm, err)
			}
		}
	}
}
...
func (app *CommonApp) initialize(data appData) {
	log.V(lvl.DEBUG).Info("initialize:path =", data.path)
...
func (app *CommonApp) translateCreate(d *db.DB) ([]db.WatchKeys, error) {
	...
	log.V(lvl.DEBUG).Info("translateCreate:path =", app.pathInfo.Path)
...
func (app *CommonApp) translateUpdate(d *db.DB) ([]db.WatchKeys, error) {
	var err error
	var keys []db.WatchKeys
	log.V(lvl.DEBUG).Info("translateUpdate:path =", app.pathInfo.Path)
...
func (app *CommonApp) translateReplace(d *db.DB) ([]db.WatchKeys, error) {
	var err error
	var keys []db.WatchKeys
	log.V(lvl.DEBUG).Info("translateReplace:path =", app.pathInfo.Path)
...
func (app *CommonApp) translateDelete(d *db.DB) ([]db.WatchKeys, error) {
	var err error
	var keys []db.WatchKeys
	log.V(lvl.DEBUG).Info("translateDelete:path =", app.pathInfo.Path)
...
func (app *CommonApp) translateGet(dbs [db.MaxDB]*db.DB) error {
	var err error
	log.V(lvl.DEBUG).Info("translateGet:path =", app.pathInfo.Path)
	return err
}
...
func (app *CommonApp) translateSubscribe(req translateSubRequest) (translateSubResponse, error) {
	...
	if uriPath, err := ygot.PathToString(subsReqXlateInfo.TrgtPathInfo.Path); err == nil {
		if log.V(4) {
			log.Warning(reqIdLogStr, "translateSubscribe: subsReqXlateInfo.TrgtPathInfo.path: ", uriPath)
		}
	} else {
		if log.V(4) {
...
func (app *CommonApp) translateAction(dbs [db.MaxDB]*db.DB) error {
	var err error
	log.V(lvl.DEBUG).Info("translateAction:path =", app.pathInfo.Path, app.body)
	return err
}
...
func (app *CommonApp) processCreate(d *db.DB) (SetResponse, error) {
	...
	log.V(lvl.DEBUG).Info("processCreate:path =", app.pathInfo.Path)
	targetType := reflect.TypeOf(*app.ygotTarget)
	log.V(lvl.DEBUG).Infof("processCreate: Target object is a <%s> of Type: %s", targetType.Kind().String(), targetType.Elem().Name())
...
func (app *CommonApp) processUpdate(d *db.DB) (SetResponse, error) {
	var err error
	var resp SetResponse
	log.V(lvl.DEBUG).Info("processUpdate:path =", app.pathInfo.Path)
...
func (app *CommonApp) processReplace(d *db.DB) (SetResponse, error) {
	var err error
	var resp SetResponse
	log.V(lvl.DEBUG).Info("processReplace:path =", app.pathInfo.Path)
...
func (app *CommonApp) processDelete(d *db.DB) (SetResponse, error) {
	...
	log.V(lvl.DEBUG).Infof("processDelete:path = %s, deleteEmptyEntry = %v", app.pathInfo.Path, app.deleteEmptyEntry)
...
func (app *CommonApp) processGet(dbs [db.MaxDB]*db.DB, fmtType TranslibFmtType) (GetResponse, error) {
	...
	var payload []byte
	log.V(lvl.DEBUG).Info("processGet:path =", app.pathInfo.Path)
	txCache := new(sync.Map)
	...
	for {
		...
		if err != nil {
			log.V(lvl.WARNING).Info("transformer.NewQueryParams() returned : ", err)
			resp.Payload = []byte("{}")
			break
		}
		...
		if err != nil {
			if err.Error() == transformer.QUERY_CONTENT_MISMATCH_ERR {
				log.V(lvl.DEBUG).Infof("GetAndXlateFromDB returned QUERY_CONTENT_MISMATCH for qParams %v on path %v", qParams.String(), app.pathInfo.Path)
				err = nil
			}
			/* Google: The following code is no longer required after adding the if check above.
			...
			*/
			if err != nil {
				log.V(lvl.WARNING).Info("transformer.GetAndXlateFromDB() returned : ", err)
			}
			resp.Payload = payload
			break
		}
		if isSonicUri && isEmptyPayload {
			log.V(lvl.DEBUG).Info("transformer.GetAndXlateFromDB() returned EmptyPayload")
			resp.Payload = payload
			break
		}
		...
		if !tgtObjCastOk {
			...
			log.V(lvl.DEBUG).Infof("Use GetParentNode() instead of casting ygotTarget to GoStruct, URI - %v", app.pathInfo.Path)
			...
			if getParentNodeErr != nil {
				log.V(lvl.WARNING).Infof("getParentNode() failure for URI %v", app.pathInfo.Path)
				resp.Payload = payload
				break
			}
			if parentTargetObj != nil {
				...
				if !tgtObjCastOk {
					log.V(lvl.WARNING).Infof("Casting of parent object returned from getParentNode() to GoStruct failed(uri - %v)", app.pathInfo.Path)
					resp.Payload = payload
					break
				}
			} else {
				log.V(lvl.WARNING).Infof("getParentNode() returned a nil Object for URI %v", app.pathInfo.Path)
				resp.Payload = payload
				break
			}
		}
		if targetObj != nil {
			if isSonicUri {
				...
				if err != nil {
					log.V(lvl.WARNING).Info("ocbinds.Unmarshal()  returned : ", err)
					resp.Payload = payload
					break
				}
			}
			...
			if !isSonicUri {
				if isEmptyPayload {
					if areEqual(appYgotStruct, origXfmrYgotRoot) {
						log.V(lvl.DEBUG).Info("origXfmrYgotRoot and appYgotStruct are equal.")
						...
						if transformer.IsLeafNode(app.pathInfo.Path) {
							...
							resp.Payload = payload
							log.V(lvl.INFO).Info("No data found for leaf.")
							err = tlerr.NotFound("Resource not found")
							break
						}
						if !qParams.IsEnabled() {
							resp.Payload = payload
							log.V(lvl.DEBUG).Info("No data available")
			...
			if resYgot != nil {
				...
				if err != nil {
					log.V(lvl.WARNING).Info("generateGetResponse() couldn't generate payload.")
					resp.Payload = payload
				}
			} else {
				resp.Payload = payload
			...
			log.V(lvl.WARNING).Info("processGet. targetObj is null. Unable to Unmarshal payload")
			resp.Payload = payload
			break
		}
	}
	if m := utils.FindYGRootMeta((*app.ygotRoot).(ygot.GoStruct)); m != nil {
		resp.YGSMeta = m.YGSMetaMap()
	}
	return resp, err
}
...
func (app *CommonApp) processAction(dbs [db.MaxDB]*db.DB) (ActionResponse, error) {
	...
	log.V(lvl.DEBUG).Info("Before calling transformer.CallRpcMethod() for path ", app.pathInfo.Path)
	...
	if log.V(5) {
		...
		log.V(lvl.INFO).Info("After calling transformer.CallRpcMethod()")
	}
...
func (app *CommonApp) processSubscribe(param processSubRequest) (processSubResponse, error) {
	...
	if err != nil {
		log.V(lvl.WARNING).Info("processSubscribe: Error in getting the NewSubscribeNotfRespXlator; error: ", err)
		return resp, err
	}
	if log.V(4) {
	...
	if resp.path, err = subNotfRespXlator.Translate(); err != nil {
		log.V(lvl.DEBUG).Info("processSubscribe: Error in translating the subscribe notification; error: ", err)
		return resp, err
	}
	return resp, nil
}
...
func (app *CommonApp) translateCRUDCommon(d *db.DB, opcode int) ([]db.WatchKeys, error) {
	...
	txCache := new(sync.Map)
	log.V(lvl.DEBUG).Info("translateCRUDCommon:path =", app.pathInfo.Path)
	...
	log.V(lvl.DEBUG).Info("transformer.XlateToDb() returned result DB map - ", result, "\nDefault value DB Map - ", defValMap, "\nAux DB Map - ", auxMap)
	...
	if len(result) == 0 {
		log.V(lvl.DEBUG).Info("XlatetoDB() returned empty map")
	...
	if (err != nil) || (len(moduleNm) == 0) {
		log.V(lvl.WARNING).Info("GetModuleNmFromPath() couldn't fetch module name.")
		return keys, err
	}
	...
	log.V(lvl.DEBUG).Info("Result Tables List", resultTblList)

	// Get list of tables to watch
	if len(resultTblList) > 0 {
		depTbls := transformer.GetTablesToWatch(resultTblList, moduleNm)
		if len(depTbls) == 0 {
			log.V(lvl.WARNING).Infof("Couldn't get Tables to watch for module %v", moduleNm)
...
func (app *CommonApp) processCommon(d *db.DB, opcode int) error {
	...
	log.V(lvl.DEBUG).Info("Proceeding to perform DB operation")
	...
	if _, ok := app.cmnAppTableMap[DELETE][db.ConfigDB]; ok {
		...
		if err != nil {
			log.V(lvl.INFO).Info("Process delete fail. cmnAppDelDbOpn error:", err)
			return err
		}
	}
	...
	if _, ok := app.cmnAppTableMap[CREATE][db.ConfigDB]; ok {
		...
		if err != nil {
			log.V(lvl.INFO).Info("Process create fail. cmnAppCRUCommonDbOpn error:", err)
			return err
		}
	}
	...
	if _, ok := app.cmnAppTableMap[UPDATE][db.ConfigDB]; ok {
		...
		if err != nil {
			log.V(lvl.INFO).Info("Process update fail. cmnAppCRUCommonDbOpn error:", err)
			return err
		}
	}
	if _, ok := app.cmnAppTableMap[REPLACE][db.ConfigDB]; ok {
		...
		if err != nil {
			log.V(lvl.INFO).Info("Process replace fail. cmnAppCRUCommonDbOpn error:", err)
			return err
		}
	}
	log.V(lvl.DEBUG).Info("Returning from processCommon() - success")
	return err
}
...
func (app *CommonApp) cmnAppCRUCommonDbOpn(d *db.DB, opcode int, dbMap map[string]map[string]db.Value) error {
	...
	for idx := len(resultTblLst) - 1; idx >= 0; idx-- {
		...
		log.V(lvl.DEBUG).Info("In Yang to DB map returned from transformer looking for table = ", tblNm)
		if tblVal, ok := dbMap[tblNm]; ok {
			cmnAppTs = &db.TableSpec{Name: tblNm}
			log.V(lvl.DEBUG).Info("Found table entry in YANG to DB map")
			if (tblVal == nil) || (len(tblVal) == 0) {
				log.V(lvl.INFO).Infof("No table instances/rows found for %s.", tblNm)
				continue
			}
			...
			log.V(lvl.DEBUG).Infof("CRU case - ordered list of DB keys for tbl %v = %v", tblNm, reverOrdDbKeyLst)

			for _, tblKey := range reverOrdDbKeyLst {
				tblRw := tblVal[tblKey]
				log.V(lvl.DEBUG).Info("Processing Table key ", tblKey)
				...
				case CREATE:
					if existingEntry.IsPopulated() {
						log.V(lvl.DEBUG).Info("Create case - Entry ", tblKey, " already exists hence modifying it.")
						...
						*/
						...
						log.V(lvl.DEBUG).Info("Processing Table row ", resTblRw)
						err = d.ModEntry(cmnAppTs, db.Key{Comp: []string{tblKey}}, resTblRw)
						if err != nil {
							log.V(lvl.WARNING).Info("CREATE case - d.ModEntry() failure")
							return err
						}
					} else {
						if tblRwDefaults, defaultOk := app.cmnAppYangDefValMap[tblNm][tblKey]; defaultOk {
							log.V(lvl.DEBUG).Info("Entry ", tblKey, " doesn't exist so fill yang defined defaults - ", tblRwDefaults)
						...
						log.V(lvl.DEBUG).Info("Processing Table row ", tblRw)
						err = d.CreateEntry(cmnAppTs, db.Key{Comp: []string{tblKey}}, tblRw)
						if err != nil {
							log.V(lvl.WARNING).Info("CREATE case - d.CreateEntry() failure")
							return err
						}
					}
				case UPDATE:
					if existingEntry.IsPopulated() {
						log.V(lvl.DEBUG).Info("Entry already exists hence modifying it.")
						/* Handle leaf-list merge if any leaf-list exists
						A leaf-list field in redis has "@" suffix as per swsssdk convention.
						*/
						resTblRw := db.Value{Field: map[string]string{}}
						...
						err = d.ModEntry(cmnAppTs, db.Key{Comp: []string{tblKey}}, resTblRw)
						if err != nil {
							log.V(lvl.WARNING).Info("UPDATE case - d.ModEntry() failure")
							return err
						}
					} else {
						// workaround to patch operation from CLI
						log.V(lvl.DEBUG).Info("Create(patch) an entry.")
						if tblRwDefaults, defaultOk := app.cmnAppYangDefValMap[tblNm][tblKey]; defaultOk {
							log.V(lvl.DEBUG).Info("Entry ", tblKey, " doesn't exist so fill defaults - ", tblRwDefaults)
							for fld, val := range tblRwDefaults.Field {
								if _, fldOk := tblRw.Field[fld]; !fldOk {
									tblRw.Field[fld] = val
								}
						...
						err = d.CreateEntry(cmnAppTs, db.Key{Comp: []string{tblKey}}, tblRw)
						if err != nil {
							log.V(lvl.WARNING).Info("UPDATE case - d.CreateEntry() failure")
							return err
						}
					}
				case REPLACE:
					...
					if tblRwDefaults, defaultOk := app.cmnAppYangDefValMap[tblNm][tblKey]; defaultOk {
						log.V(lvl.DEBUG).Info("For entry ", tblKey, ", being replaced, fill defaults - ", tblRwDefaults)
						for fld, val := range tblRwDefaults.Field {
							if _, fldOk := tblRw.Field[fld]; !fldOk {
								tblRw.Field[fld] = val
							}
					...
					log.V(lvl.DEBUG).Info("Processing Table row ", tblRw)
					if existingEntry.IsPopulated() {
						log.V(lvl.DEBUG).Info("Entry already exists.")
						if len(origTblRw.Field) == 1 {
							...
							if isLeafListFld && (app.cmnAppOpcode == REPLACE) && (transformer.IsLeafListNode(app.pathInfo.Path)) {
								log.V(lvl.DEBUG).Info("For entry ", tblKey, ", field ", fldNm, " will have value ", fldVal)
								err = d.ModEntry(cmnAppTs, db.Key{Comp: []string{tblKey}}, origTblRw)
								if err != nil {
									log.V(lvl.WARNING).Info("REPLACE case - d.ModEntry() failure")
									return err
								}
						...
						log.V(lvl.DEBUG).Info("Process Aux row ", auxRw)
						isTlNd := false
						if !strings.HasPrefix(app.pathInfo.Path, "/sonic") {
							isTlNd, err = transformer.IsTerminalNode(app.pathInfo.Path)
							log.V(lvl.DEBUG).Info("transformer.IsTerminalNode() returned - ", isTlNd, " error ", err)
							if err != nil {
								return err
							}
						}
						if isTlNd && isPartialReplace(existingEntry, tblRw, auxRw) {
							log.V(lvl.DEBUG).Info("Since its partial replace modifying fields - ", tblRw)
							...
							if auxRwOk {
								if len(auxRw.Field) > 0 {
									log.V(lvl.DEBUG).Info("Since its partial replace delete aux fields - ", auxRw)
									err := d.DeleteEntryFields(cmnAppTs, db.Key{Comp: []string{tblKey}}, auxRw)
									if err != nil {
										log.V(lvl.WARNING).Info("REPLACE case - d.DeleteEntryFields() failure")
										return err
									}
								}
							}
							...
							if err != nil {
								log.V(lvl.WARNING).Info("REPLACE case - d.SetEntry() failure")
								return err
							}
						}
					} else {
						log.V(lvl.DEBUG).Info("Entry doesn't exist hence create it.")
						err = d.CreateEntry(cmnAppTs, db.Key{Comp: []string{tblKey}}, tblRw)
						if err != nil {
							log.V(lvl.WARNING).Info("REPLACE case - d.CreateEntry() failure")
							return err
						}
					}
				}
...
func (app *CommonApp) cmnAppDelDbOpn(d *db.DB, opcode int, dbMap map[string]map[string]db.Value) error {
	...
	moduleNm, err = transformer.GetModuleNmFromPath(app.pathInfo.Path)
	if (err != nil) || (len(moduleNm) == 0) {
		log.V(lvl.WARNING).Info("GetModuleNmFromPath() failed")
		return err
	}
	log.V(lvl.DEBUG).Info("getModuleNmFromPath() returned module name = ", moduleNm)
	...
	for _, tblNm := range resultTblLst {
		...
		if tblVal, ok := dbMap[tblNm]; ok {
			...
			if !app.skipOrdTableChk {
				...
				if len(ordTblList) == 0 {
					log.V(lvl.WARNING).Info("GetOrdTblList returned empty slice")
					err = errors.New("GetOrdTblList returned empty slice. Insufficient information to process request")
					return err
				}
				log.V(lvl.DEBUG).Infof("GetOrdTblList for table - %v, module %v returns %v", tblNm, moduleNm, ordTblList)
			}
			if len(tblVal) == 0 {
				log.V(lvl.DEBUG).Info("DELETE case - No table instances/rows found hence delete entire table = ", tblNm)
				if !app.skipOrdTableChk {
					for _, ordtbl := range ordTblList {
						...
						log.V(lvl.DEBUG).Info("Since parent table is to be deleted, first deleting child table = ", ordtbl)
						dbTblSpec = &db.TableSpec{Name: ordtbl}
						err = d.DeleteTable(dbTblSpec)
						if err != nil {
							log.V(lvl.WARNING).Info("DELETE case - d.DeleteTable() failure for Table = ", ordtbl)
							return err
						}
					}
				}
				...
				if err != nil {
					log.V(lvl.WARNING).Info("DELETE case - d.DeleteTable() failure for Table = ", tblNm)
					return err
				}
				log.V(lvl.DEBUG).Info("DELETE case - Deleted entire table = ", tblNm)
			...
			ordDbKeyLst := transformer.SortSncTableDbKeys(tblNm, tblVal)
			log.V(lvl.DEBUG).Infof("DELETE case - ordered list of DB keys for tbl %v = %v", tblNm, ordDbKeyLst)

			for _, tblKey := range ordDbKeyLst {
				tblRw := tblVal[tblKey]
				if len(tblRw.Field) == 0 {
					log.V(lvl.DEBUG).Info("DELETE case - no fields/cols to delete hence delete the entire row.")
					log.V(lvl.DEBUG).Info("First, delete child table instances that correspond to parent table instance to be deleted = ", tblKey)
					if !app.skipOrdTableChk {
						for _, ordtbl := range ordTblList {
							if ordtbl == tblNm {
								// Handle the child tables only till you reach the parent table entry
							...
							dbTblSpec = &db.TableSpec{Name: ordtbl}
							keyPattern := tblKey + "|*"
							log.V(lvl.DEBUG).Info("Key pattern to be matched for deletion = ", keyPattern)
							err = d.DeleteKeys(dbTblSpec, db.Key{Comp: []string{keyPattern}})
							if err != nil {
								log.V(lvl.WARNING).Info("DELETE case - d.DeleteTable() failure for Table = ", ordtbl)
								return err
							}
							log.V(lvl.DEBUG).Info("Deleted keys matching parent table key pattern for child table = ", ordtbl)
						}
					}
					...
					if err != nil {
						...
						case tlerr.TranslibCVLFailure:
							if cvl.CVLRetCode(e.Code) == cvl.CVL_SEMANTIC_KEY_NOT_EXIST {
								log.V(lvl.INFO).Infof("Ignore delete that cannot be processed for table %v key %v that does not exist. err %v", tblNm, tblKey, e.CVLErrorInfo.ConstraintErrMsg)
								err = nil
							} else {
								log.V(lvl.WARNING).Info("DELETE case - d.DeleteEntry() failure")
								return err
							}
						default:
							log.V(lvl.WARNING).Info("DELETE case - d.DeleteEntry() failure")
					...
					log.V(lvl.DEBUG).Info("Finally deleted the parent table row with key = ", tblKey)
				} else {
					...
					log.V(lvl.DEBUG).Info("DELETE case - fields/cols to delete hence delete only those fields.")
					existingEntry, exstErr := d.GetEntry(cmnAppTs, db.Key{Comp: []string{tblKey}})
					if exstErr != nil {
						log.V(lvl.INFO).Info("Table Entry from which the fields are to be deleted does not exist. Ignore error for non existant instance for idempotency")
						continue
					}
					/* handle leaf-list merge if any leaf-list exists */
					resTblRw := checkAndProcessLeafList(existingEntry, tblRw, DELETE, d, tblNm, tblKey)
					log.V(lvl.DEBUG).Info("DELETE case - checkAndProcessLeafList() returned table row ", resTblRw)
					if len(resTblRw.Field) > 0 {
						if !app.deleteEmptyEntry {
							...
							if deleteCount == len(existingEntry.Field) {
								...
								log.V(lvl.DEBUG).Info("Last field gets deleted, add NULL field to keep an db entry")
						...
						if err != nil {
							log.V(lvl.WARNING).Info("DELETE case - d.DeleteEntryFields() failure")
							return err
						}
					}
				}
...
func checkAndProcessLeafList(existingEntry db.Value, tblRw db.Value, opcode int, d *db.DB, tblNm string, tblKey string) db.Value {
	...
	for field, value := range tblRw.Field {
		if strings.HasSuffix(field, "@") {
			...
			log.V(lvl.DEBUG).Infof("Existing DB value for field %v - %v", field, exstLst)
			var valueLst []string
			if value != "" { //zero len string as leaf-list value is treated as delete entire leaf-list
				valueLst = strings.Split(value, ",")
			}
			log.V(lvl.DEBUG).Infof("Incoming value for field %v - %v", field, valueLst)
			if len(exstLst) != 0 {
				log.V(lvl.DEBUG).Infof("Existing list is not empty for field %v", field)
				...
				log.V(lvl.DEBUG).Infof("For field %v value after merging incoming with existing %v", field, exstLst)
				if opcode == DELETE {
					if len(valueLst) > 0 {
				...
				log.V(lvl.DEBUG).Infof("Existing list is empty for field %v", field)
				if opcode == UPDATE {
					if len(valueLst) > 0 {
						exstLst = valueLst
						tblRw.SetList(field, exstLst)
	...
	if opcode == DELETE {
		if len(mergeTblRw.Field) == 0 {
			log.V(lvl.DEBUG).Infof("mergeTblRow is empty - Returning Table Row %v", tblRw)
			return tblRw
		}
		err := d.ModEntry(dbTblSpec, db.Key{Comp: []string{tblKey}}, mergeTblRw)
		if err != nil {
			log.V(lvl.WARNING).Info("DELETE case(merge leaf-list) - d.ModEntry() failure")
		}
	}
	log.V(lvl.DEBUG).Infof("Returning Table Row %v", tblRw)
	return tblRw
}
...
func isPartialReplace(exstRw db.Value, replTblRw db.Value, auxRw db.Value) bool {
	...
	*/
	...
	for exstFld := range exstRw.Field {
		...
		if !isIncomingFld {
			log.V(lvl.DEBUG).Info("Entry contains field ", exstFld, " not found in result, default and aux fields hence its partial replace.")
			partialReplace = true
			break
		}
	}
	log.V(lvl.DEBUG).Info("returning partialReplace - ", partialReplace)
