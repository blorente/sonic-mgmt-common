...
func (pv *pathValidator) Validate(gPath *gnmi.Path) error {
	if pv == nil {
		log.V(lvl.WARNING).Infof("error: pathValidator is nil")
		return fmt.Errorf("pathValidator is nil")
	}
	if gPath == nil {
		log.V(lvl.WARNING).Infof("error: gnmi path nil")
	...
	if pv.err = pv.validatePath(); pv.err != nil {
		log.V(lvl.WARNING).Infof("error in validating the path: %v", pv.err)
	}
	return pv.err
}
...
func (pv *pathValidator) getYangSchema() (*yang.Entry, error) {
	...
	if sVal.Elem().Type().Kind() == reflect.Struct {
		...
		if ygSchema == nil {
			log.V(lvl.WARNING).Infof("error: ygot object name %v not found in the schema for the given path: %v", objName, pv.gPath)
...
func (pv *pathValidator) validateListKeyValues(schema *yang.Entry, gPath *gnmi.Path) error {
	...
	if err != nil {
		log.V(lvl.WARNING).Infof("error in GetOrCreateNode: %v", err)
