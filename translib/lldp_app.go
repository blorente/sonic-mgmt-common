...
func init() {
	log.V(lvl.INFO).Info("Init called for LLDP modules module")
...
func (app *lldpApp) initialize(data appData) {
	log.V(lvl.INFO).Info("initialize:lldp:path =", data.path)
...
func (app *lldpApp) translateCreate(d *db.DB) ([]db.WatchKeys, error) {
	...
	log.V(lvl.INFO).Info("translateCreate:lldp:path =", app.path)
...
func (app *lldpApp) translateUpdate(d *db.DB) ([]db.WatchKeys, error) {
	var err error
	var keys []db.WatchKeys
	log.V(lvl.INFO).Info("translateUpdate:lldp:path =", app.path)
...
func (app *lldpApp) translateReplace(d *db.DB) ([]db.WatchKeys, error) {
	var err error
	var keys []db.WatchKeys
	log.V(lvl.INFO).Info("translateReplace:lldp:path =", app.path)
...
func (app *lldpApp) translateDelete(d *db.DB) ([]db.WatchKeys, error) {
	var err error
	var keys []db.WatchKeys
	log.V(lvl.INFO).Info("translateDelete:lldp:path =", app.path)
...
func (app *lldpApp) translateGet(dbs [db.MaxDB]*db.DB) error {
	var err error
	log.V(lvl.INFO).Info("translateGet:lldp:path = ", app.path)
...
func (app *lldpApp) processGet(dbs [db.MaxDB]*db.DB, fmtType TranslibFmtType) (GetResponse, error) {
	...
	log.V(lvl.INFO).Info("lldp processGet")
	log.V(lvl.INFO).Info("targetUriPath: ", targetUriPath)

	if targetUriPath == "/openconfig-lldp:lldp/interfaces" {
		log.V(lvl.INFO).Info("Requesting interfaces")
		...
		for ifname, _ := range app.lldpNeighTableMap {
			...
			if err != nil {
				log.V(lvl.INFO).Info("Creation of subinterface subtree failed!")
		...
		if intfObj.Interface != nil && len(intfObj.Interface) > 0 {
			for ifname, _ := range intfObj.Interface {
				log.V(lvl.INFO).Info("if-name = ", ifname)
...
func (app *lldpApp) getLldpNeighInfoFromInternalMap(ifName *string, ifInfo *ocbinds.OpenconfigLldp_Lldp_Interfaces_Interface) {
	...
	for attr, value := range neighAttrMap {
		...
		default:
			log.V(lvl.INFO).Info("Not a valid attribute!")
		}
	}
...
func (app *lldpApp) getLldpInfoFromDB(ifname *string) {
	...
	if err != nil {
		log.V(lvl.INFO).Info("Can't get lldp table")
		return
	}

	keys, err := lldpTbl.GetKeys()
	if err != nil {
		log.V(lvl.INFO).Info("Can't get lldp keys")
		return
	}

	for _, key := range keys {
		log.V(lvl.INFO).Info("lldp key = ", key.Get(0))

		lldpEntry, err := app.appDb.GetEntry(app.neighTs, db.Key{Comp: []string{key.Get(0)}})
		if err != nil {
			log.V(lvl.INFO).Info("can't access neighbor table for key: ", key.Get(0))
			return
		}

		if lldpEntry.IsPopulated() {
			log.V(lvl.INFO).Info("lldp entry populated for key: ", key.Get(0))
	...
	for _, key := range keys {
		...
		for lldpAttr := range entryData.Field {
			...
			default:
				log.V(lvl.INFO).Info("Unknown LLDP Attribute")
			}
		}
	}
}
...
func (app *lldpApp) getRemoteSysCap(capb string, ifname string, setCap bool) {
	...
	log.V(lvl.INFO).Info("sysCap: ", sysCap)
	...
	if (sysCap & (128 >> 1)) != 0 {
		...
		if setCap {
			log.V(lvl.INFO).Info("Repeater ENABLED")
	...
	if (sysCap & (128 >> 2)) != 0 {
		...
		if setCap {
			log.V(lvl.INFO).Info("Bridge  ENABLED")
	...
	if (sysCap & (128 >> 4)) != 0 {
		...
		if setCap {
			log.V(lvl.INFO).Info("Router ENABLED")
