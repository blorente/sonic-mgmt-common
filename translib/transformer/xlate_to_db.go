...
func mapFillData(xlateParams xlateToParams) error {
	...
	if !ok || xpathInfo == nil {
		log.V(lvl.WARNING).Infof("Yang path(\"%v\") not found.", xpath)
		return nil
	}

	if xpathInfo.tableName == nil && xpathInfo.xfmrTbl == nil {
		log.V(lvl.WARNING).Infof("Table for yang-path(\"%v\") not found.", xpath)
		return nil
	}

	if xpathInfo.tableName != nil && *xpathInfo.tableName == XFMR_NONE_STRING {
		log.V(lvl.WARNING).Infof("Table for yang-path(\"%v\") NONE.", xpath)
		return nil
	}

	if len(xlateParams.keyName) == 0 {
		log.V(lvl.WARNING).Infof("Table key for YANG path(\"%v\") not found.", xpath)
		return nil
	}
	...
	if xpathInfo.xfmrTbl != nil {
		...
		if tableName == "" {
			log.V(lvl.WARNING).Infof("No table name found for URI (\"%v\")", xlateParams.uri)
			return err
		}
	} else {
		...
		if _, fieldOk := xDbSpecMap[fieldXpath]; fieldOk && dbEntry != nil {
			...
			if dbEntry.IsLeafList() {
				...
				for fidx := 0; fidx < fieldDt.Len(); fidx++ {
					...
					if err != nil {
						log.V(lvl.WARNING).Infof("Couldn't unmarshal Json to DbData: path(\"%v\") error (\"%v\").", fieldXpath, err)
					} else {
...
func dbMapDataFillForNestedList(tableName string, key string, nestedListDbEntry *yang.Entry, jsonData interface{}, result map[string]map[string]db.Value) {
	...
	*/
	...
	for idx := 0; idx < nestedListData.Len(); idx++ {
		...
		if err != nil {
			log.V(lvl.WARNING).Infof("Couldn't unmarshal Json to DbData: path(\"%v\"), value %v,  error (\"%v\").", fieldXpath, nestedListInstance[nestedListYangKeyName], err)
			continue
		}
		for field, value := range nestedListInstance {
			...
			if err != nil {
				log.V(lvl.WARNING).Infof("Couldn't unmarshal Json to DbData: path(\"%v\"), value %v error (\"%v\").", fieldValXpath, value, err)
				break
			} else {
...
func dbMapUpdate(d *db.DB, ygRoot *ygot.GoStruct, oper Operation, path string, requestUri string, jsonData interface{}, result map[Operation]map[db.DBNum]map[string]map[string]db.Value, yangDefValMap map[string]map[string]db.Value, yangAuxValMap map[string]map[string]db.Value, txCache interface{}) error {
	...
	if oper == REPLACE {
		...
		module := pathList[0]
		// TODO(b/333950424) Top level container replace appears to work fine; upstream the removal of this check.
		if (false && len(pathList) == 1) && ((strings.Contains(module, ":")) && (strings.HasPrefix(module, OC_MDL_PFX) || strings.HasPrefix(module, IETF_MDL_PFX) || strings.HasPrefix(module, IANA_MDL_PFX) || strings.HasPrefix(module, SONIC_MDL_PFX))) {
					...
					if (childNode.tableName != nil && *childNode.tableName == tblName) || (childNode.xfmrTbl != nil) {
						...
						if !ok {
							if len(childNode.xfmrField) > 0 {
								...
								if err != nil {
									log.V(lvl.WARNING).Infof("Default/AuxMap Value filling. Received error %v from %v", err, childNode.xfmrField)
								}
								if retData != nil {
								...
								if xDbSpecInfo, ok := xDbSpecMap[tblName+"/"+childNode.fieldName]; ok && (xDbSpecInfo != nil) && (!xDbSpecInfo.isKey) {
									...
									if !ok {
										if len(childNode.defVal) > 0 {
											...
											err := mapFillDataUtil(curXlateParams)
											if err != nil {
												log.V(lvl.WARNING).Infof("Default/AuxMap Value filling. Received error %v from %v", err, childNode.fieldName)
											}
										} else {
											if xlateParams.oper != REPLACE {
												continue
...
func dbMapCreate(d *db.DB, ygRoot *ygot.GoStruct, oper Operation, uri string, requestUri string, jsonData interface{}, resultMap map[Operation]RedisDbMap, yangDefValMap map[string]map[string]db.Value, yangAuxValMap map[string]map[string]db.Value, txCache interface{}) error {
	...
	if err != nil {
		log.V(lvl.WARNING).Infof("Cannot perform Operation %v on URI %v due to - %v", oper, uri, err)
		return err
	}
	if !exists {
	...
	if isSonicYang(uri) {
				...
				if ok {
					...
					if dbFldok {
						if dbSpecInfo.yangType == YANG_LEAF {
							...
							log.V(lvl.WARNING).Infof("For URI - %v, unrecognized terminal YANG node type", uri)
						}
					} else if !dbFldok {
						...
						if ok && dbSpecNestedChildInfo != nil {
							if dbSpecNestedChildInfo.yangType == YANG_LIST && dbSpecNestedChildInfo.dbEntry.Parent.IsList() { //nested list case
								...
								log.V(lvl.WARNING).Infof("For URI - %v, only nested list supported, other type of yang node not supported - %v", uri, dbSpecPath)
							}
						} else {
							log.V(lvl.WARNING).Infof("For URI - %v, no entry found in xDbSpecMap for table(%v)/field(%v)", uri, tableName, fldNm)
						}

					}
				} else {
					log.V(lvl.WARNING).Infof("For URI - %v, no entry found in xDbSpecMap with tableName - %v", uri, tableName)
				}
			}
		} else {
		...
		if xYangModSpecMap != nil {
			if modSpecInfo, specOk := xYangModSpecMap[moduleNm]; specOk && (len(modSpecInfo.xfmrPre) > 0) {
				...
				if err != nil {
					log.V(lvl.WARNING).Infof("Pre-transformer: %v failed.(err:%v)", modSpecInfo.xfmrPre, err)
					return err
				}
			}
		}
	...
	if err == nil {
		if !isSonicYang(uri) {
			...
			if _, ok := xYangSpecMap[moduleNm]; ok {
				...
				log.V(lvl.WARNING).Infof("No Entry exists for module %s in xYangSpecMap. Unable to process post xfmr (\"%v\") uri(\"%v\") error (\"%v\").", oper, uri, err)
			}
		...
		if err != nil {
			log.V(lvl.WARNING).Infof("Failed in dbdata-xfmr for %v", resultMap)
			return err
		}
		...
		if err != nil {
			log.V(lvl.WARNING).Infof("Failed in dbdata-xfmr for %v", yangDefValDbDataXfmrMap)
			return err
		}
		...
		if err != nil {
			log.V(lvl.WARNING).Infof("Failed in dbdata-xfmr for %v", yangAuxValDbDataXfmrMap)
			return err
		}
		...
		log.V(lvl.WARNING).Infof("DBMapCreate req failed for oper (\"%v\") uri(\"%v\") error (\"%v\").", oper, uri, err)
	}
	return err
}
...
func yangNodeForUriGet(uri string, ygRoot *ygot.GoStruct) (interface{}, error) {
	...
	if path == nil || err != nil {
		log.V(lvl.WARNING).Infof("For URI %v - StringToPath failure", uri)
	...
	if (node == nil) || (len(node) == 0) || (node[0].Data == nil) {
		log.V(lvl.WARNING).Infof("GetNode returned nil for URI %v", uri)
					...
					if ok && strings.HasPrefix(curXpath, reqXpath) {
						if (!xYangSpecMap[xpath].isKey) || (len(xYangSpecMap[xpath].xfmrField) > 0) || (xYangSpecMap[xpath].isKey && strings.HasPrefix(xlateParams.uri, "/"+IETF_MDL_PFX)) {
							if len(xYangSpecMap[xpath].xfmrFunc) == 0 {
								...
								if retErr != nil {
									log.V(lvl.WARNING).Infof("Failed constructing data for db write: key(\"%v\"), value(\"%v\"), path(\"%v\").",
...
func verifyParentTableSonic(d *db.DB, dbs [db.MaxDB]*db.DB, oper Operation, uri string, dbData RedisDbMap) (bool, error) {
	...
	if (len(table) > 0) && (len(dbKey) > 0) {
		...
		if oper == GET {
			...
			if !ok {
				log.V(lvl.WARNING).Infof("No entry in xDbSpecMap for xpath %v, for URI - %v", table, uri)
			} else {
				cdb = dbInfo.dbIndex
			}
		...
		if len(pathList) == SONIC_TBL_CHILD_INDEX && (oper == UPDATE || oper == CREATE || oper == DELETE || oper == GET) && !tableExists {
			...
			log.V(lvl.WARNING).Infof("Parent table %v with key %v does not exist for oper %v in DB", table, dbKey, oper)
			...
			log.V(lvl.WARNING).Infof("Parent table %v with key %v does not exist in DB", table, dbKey)
			err = tlerr.NotFound("Resource not found")
			return false, err
		} else {
...
func verifyParentTblSubtree(dbs [db.MaxDB]*db.DB, uri string, requestURI string, xfmrFuncNm string, oper Operation, dbData RedisDbMap) (bool, error) {
	var inParams XfmrSubscInParams
	inParams.uri = uri
	inParams.requestURI = requestURI
	inParams.dbDataMap = make(RedisDbMap)
	inParams.dbs = dbs
	/* Google: This is probably supposed to be TRANSLATE_EXISTS instead of TRANSLATE_SUBSCRIBE.
	 * That makes sense based on what we are doing here (checking for table & key existence) and
	 * the oc-interface subscribe xfmr upstreamed also has comments indicating the EXISTS mode
	 * should be used.
	 * https://github.com/sonic-net/sonic-mgmt-common/blob/966adc0d92f7c5e2f77636288653489bb97d9deb/translib/transformer/xfmr_intf.go#L2571-L2577
	...
	*/
	...
	if st_err != nil {
			...
			for table, keyInstance := range dbMap {
				...
				for dbKey := range keyInstance {
					...
					if oper != GET {
						...
						if err != nil {
							log.V(lvl.WARNING).Infof("Couldn't allocate NewDb/DbOptions for db - %v, while processing URI - %v", dbNo, uri)
							parentTblExists = false
							goto Exit
						}
					...
					if !exists || err != nil {
						log.V(lvl.WARNING).Infof("Parent Tbl :%v, dbKey: %v does not exist for URI %v", table, dbKey, uri)
						err = tlerr.NotFound("Resource not found")
						parentTblExists = false
						goto Exit
					}
		...
		log.V(lvl.WARNING).Infof("No Table information retrieved from subtree for URI %v", uri)
		err = tlerr.NotFound("Resource not found")
		parentTblExists = false
		goto Exit
	}
Exit:
	xfmrLogDebug("For subtree at URI - %v, returning, parentTblExists - %v, err - %v", uri, parentTblExists, err)
...
func verifyParentTableOc(d *db.DB, dbs [db.MaxDB]*db.DB, ygRoot *ygot.GoStruct, oper Operation, uri string, dbData RedisDbMap, txCache interface{}, subOpDataMap map[Operation]*RedisDbMap, dbTblKeyCache map[string]tblKeyCache) (bool, error) {
	...
	for idx, path := range parentUriList {
		...
		if (pathInfo != nil) && (len(pathInfo.Vars) > 0) {
			...
			if len(curXpathInfo.xfmrFunc) > 0 {
				...
				stParentTblExists, err = verifyParentTblSubtree(dbs, curUri, uri, curXpathInfo.xfmrFunc, oper, dbData)
				if err != nil {
					parentTblExists = false
					break
				}
				if !stParentTblExists {
					log.V(lvl.WARNING).Infof("Parent Table does not exist for URI %v", uri)
					err = tlerr.NotFound("Resource not found")
					parentTblExists = false
					break
				}
				...
				if xerr != nil {
					log.V(lvl.WARNING).Infof("Failed to get table and key for uri: %v err: %v", curUri, xerr)
					err = xerr
					parentTblExists = false
					break
				}
				...
				if len(xpathKeyExtRet.tableName) > 0 && len(xpathKeyExtRet.dbKey) > 0 {
					...
					if !existsInDbData {
						...
						if !exists {
							parentTblExists = false
							log.V(lvl.WARNING).Infof("Parent Tbl :%v, dbKey: %v does not exist for URI %v", xpathKeyExtRet.tableName, xpathKeyExtRet.dbKey, uri)
							err = tlerr.NotFound("Resource not found")
							break
						}
					}
					...
					parentTblExists = false
					log.V(lvl.WARNING).Infof("Parent Tbl :%v, dbKey: %v does not exist for URI %v", xpathKeyExtRet.tableName, xpathKeyExtRet.dbKey, curUri)
					err = tlerr.NotFound("Resource not found")
					break
				}
			}
	...
	if yangType == YANG_LIST && (oper == UPDATE || oper == CREATE || oper == DELETE || oper == GET) {
		...
		if xpathErr != nil {
			log.V(lvl.WARNING).Infof("Xpath conversion didn't happen for Uri - %v, due to - %v", uri, xpathErr)
		...
		if len(xpathInfo.xfmrFunc) > 0 {
			...
			parentTblExists, err = verifyParentTblSubtree(dbs, uri, uri, xpathInfo.xfmrFunc, oper, dbData)
			if err != nil {
				return false, err
			}
			if !parentTblExists {
		...
		if xerr != nil {
			log.V(lvl.WARNING).Infof("key extract failed err: %v, table %v, key %v, operation: %v", xerr, xpathKeyExtRet.tableName, xpathKeyExtRet.dbKey, oper)
			return false, xerr
		}
		if xpathKeyExtRet.isVirtualTbl {
			return true, nil
		...
		if !((strings.HasSuffix(uri, "]")) || (strings.HasSuffix(uri, "]/"))) { //uri points to entire list
			...
			if derr != nil {
				log.V(lvl.WARNING).Infof("ParentTable GetEntry failed for table: %v, key: %v err: %v", xpathKeyExtRet.tableName, xpathKeyExtRet.dbKey, derr)
				return false, derr
			}
			if !exists {
				log.V(lvl.WARNING).Infof("ParentTable GetEntry failed for table: %v, key: %v err: %v", xpathKeyExtRet.tableName, xpathKeyExtRet.dbKey, derr)
			...
			log.V(lvl.WARNING).Infof("Parent table check: Unable to get valid table and key err: %v, table %v, key %v. Please verify table and key mapping", xerr, xpathKeyExtRet.tableName, xpathKeyExtRet.dbKey)
			return false, xerr
		}
