...
func (respXlator *subscribeNotfRespXlator) Translate() (*gnmi.Path, error) {
	...
	for idx := len(pathElem) - 1; idx >= 0; idx-- {
		...
		if log.V(dbLgLvl) {
			log.Infof("%s subscribeNotfRespXlator:Translate: ygXpathInfo: %#v", ntfXlateReq.reqLogId, ygXpathInfo)
		}

		// Keep track of the number of keys found in the path so far.  In case
		// of wildcard queries containing a mix of * and fixed keys we may need
		// to exclude some keys when resolving the *, for example /a/b[*]/c/d[foo]
		// would need to exclude d's "foo" key when resolving b's wildcard.
		pathKeysFound = pathKeysFound + len(pathElem[idx].Key)
		...
		if ygXpathInfo.virtualTbl != nil && (*ygXpathInfo.virtualTbl) {
			log.V(lvl.WARNING).Info(ntfXlateReq.reqLogId, "Translate: virtual table is set to true and path transformer not found list node path: ", *respXlator.ntfXlateReq.path)
		...
		if len(ygXpathInfo.xfmrFunc) == 0 && len(ygXpathInfo.xfmrKey) > 0 {
			...
			if err = respXlator.processDbToYangKeyXfmrList(); err != nil {
				log.V(lvl.WARNING).Info(ntfXlateReq.reqLogId, "Translate: Error in processDbToYangKeyXfmrList for the path: ", *respXlator.ntfXlateReq.path)
				return nil, err
			}
		} else {
			...
			if dbInfo, ok := xDbSpecMap[tblName]; !ok || dbInfo == nil {
				err = fmt.Errorf("error: direct conversion from db key in db %d to yang key is not supported for the"+
				...
				log.V(lvl.WARNING).Info(ntfXlateReq.reqLogId, err)
				return nil, err
			}
			...
			if len(pathElem[idx].Key) > len(dbKeyComp) {
				log.V(lvl.WARNING).Info(ntfXlateReq.reqLogId, "Translate: Could not find the path transformer or DbToYangKey transformer for the ygXpathListInfo: ", ygPath)
			...
			// It is legal to have more db key components than yang key components; for example a path of a/b[k1=v1]/c/d[k2=v2]
			// may have two db key components but only a single yang key component at d.
			// In such a case we match the yang keys with the trailing db key components, hence the reverse walk.
			// We may have also resolved some keys already (or they were not wildcard and didn't need to be resolved)
			// so we drop those values we already used from dbKeyComp.
			numKeysUsed := pathKeysFound - len(pathElem[idx].Key)
			keys := dbKeyComp[:len(dbKeyComp)-numKeysUsed]
			log.V(lvl.DEBUG).Infof("%s allKeys=%v, numUsed=%d, keysUsed=%v", ntfXlateReq.reqLogId, dbKeyComp, numKeysUsed, keys)
			dbKeyIdx := len(keys) - 1
			for i := len(dbYgListKeyNames) - 1; i >= 0; i-- {
				dbKeyNm := dbYgListKeyNames[i]
				pathElem[idx].Key[dbKeyNm] = keys[dbKeyIdx]
				dbKeyIdx--
			}
			log.V(lvl.DEBUG).Infof("%s Resolved keys: %v", ntfXlateReq.reqLogId, pathElem[idx])
		}
	}

	log.V(lvl.DEBUG).Info(ntfXlateReq.reqLogId, "subscribeNotfRespXlator: translated path: ", *respXlator.ntfXlateReq.path)
	return respXlator.ntfXlateReq.path, nil
}
...
func (respXlator *subscribeNotfRespXlator) processDbToYangKeyXfmrList() error {
	for idx := len(respXlator.dbYgXlateList) - 1; idx >= 0; idx-- {
		if err := respXlator.dbYgXlateList[idx].handleDbToYangKeyXlate(); err != nil {
			log.V(lvl.WARNING).Infof(respXlator.ntfXlateReq.reqLogId+"handleDbToYangKeyXlate: Error: %v for the  ygPathTmp: %v ",
...
func (respXlator *subscribeNotfRespXlator) getYangXpathInfo(ygPath string) (*yangXpathInfo, error) {
	...
	if !ok || ygXpathListInfo == nil {
		log.V(lvl.WARNING).Info(respXlator.ntfXlateReq.reqLogId, "ygXpathInfo data not found in the xYangSpecMap for xpath : ", ygPath)
...
func (keyRslvr *DbYangKeyResolver) getMatchingDbYangListKeyNames(ygListKey map[string]string) ([]string, error) {
	...
	for _, listName := range ygDbInfo.listName {
		...
		// For a multi level list (e.g. /a/b[k1]/c/d[k2]/leaf) the list d may be represented as a db table
		// TBLNAME|k1|k2 causing the keylist for the ygDbListNode.Key to be "k1 k2" however, only k2 would
		// be available from the gnmi.PathElem.Key for d.  So trim the leading key names off ygDbListNode.
		numKeyFields := len(ygListKey)
		keyList := strings.Fields(ygDbListNode.Key)
		if len(keyList) > numKeyFields && numKeyFields > 0 {
			log.V(5).Infof("Trimming keyList (%v) to use only the last %v key fields", keyList, numKeyFields)
			keyList = keyList[len(keyList)-numKeyFields:]
		}
		if log.V(dbLgLvl) {
			log.Info("keyList: ", keyList)
			log.Infof("ygListKey: %#v", ygListKey)
		}
		isMatch := true
...
func (dbYgXlateInfo *DbYgXlateInfo) handleTableXfmrCallback() ([]string, error) {
	...
	if tblXfmrErr != nil {
		log.V(lvl.WARNING).Infof("%v handleTableXfmrCallback: table transformer callback returns"+
