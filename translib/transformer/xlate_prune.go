...
func xfmrPruneQP(ygRoot *ygot.GoStruct, queryParams QueryParams, uri string,
	...
	if log.V(5) {
		log.Infof("xfmrPruneQP: URI %v, requestUri %v, TimeTaken %s", uri, requestUri, tt)
...
func pruneYGObj(val reflect.Value, xpath string, queryParams *QueryParams,
	...
	if ygutil.IsValueNil(val) {
		log.V(lvl.DEBUG).Infof("pruneYGObj: Bypassing nil node: %s, %v", xpath, val)
		return nil
	}

	if !val.IsValid() {
		log.V(lvl.DEBUG).Infof("pruneYGObj: Bypassing !Valid node:%s", xpath)
		return nil
	}

	if !ygutil.IsValueStructPtr(val) {
		log.V(lvl.DEBUG).Infof("pruneYGObj: Keeping leaf(y) node: %s type: %v val: %v",
			xpath, val.Kind(), val)
		return nil
	}
	...
	for i := 0; i < fvals.NumField(); i++ {
		...
		if keep, keepSubtree := matchQueryParametersByXpath(chldXpath,
			...
			if !keep {
				log.V(lvl.DEBUG).Infof("pruneYGObj: Removing node: %s type: %v",
					chldXpath, fv.Kind())
				fv.Set(reflect.Zero(fv.Type()))
			}

			if keepSubtree {
				log.V(lvl.DEBUG).Infof("pruneYGObj: Keeping subtree: %s type: %v",
					chldXpath, fv.Kind())
			}
		...
		case reflect.Map:
			...
			if queryParams.isDepthEnabled() &&
				...
				log.V(lvl.DEBUG).Infof("pruneYGObj: Trim chldXpath: %s pruneDepth: %d",
		...
		default:
			log.V(lvl.DEBUG).Infof("pruneYGObj: Keeping node: %s type: %v",
				chldXpath, fv.Kind())
		}
	}

	if log.V(4) {
		log.Infof("pruneYGObj: Done Pruning: xpath %v, pruneDepth %d depthDiff %d",
			xpath, pruneDepth, depthDiff)
	}
	return nil
}
...
func matchQueryParametersByXpath(xpath string, queryParams *QueryParams,
	...
	if yangEntry == nil {
		...
		log.V(lvl.WARNING).Infof("matchQueryParametersByXpath: not found: %s", xpath)
		return true, true
	}
