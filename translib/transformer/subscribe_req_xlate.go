...
func (pathXltr *subscribePathXlator) setTrgtYgXpathInfo() error {
	...
	for ygXpathInfo != nil && len(ygXpathInfo.xfmrKey) == 0 {
		...
		if !ok || ygXpathInfoTmp == nil {
			log.V(lvl.WARNING).Infof("%v xYangSpecMap does not have the yangXpathInfo for the path: %v", pathXltr.subReq.reqLogId, ygPathTmp)
...
func NewSubscribeReqXlator(subReqId interface{}, reqUri string, reqMode NotificationType, dbs [db.MaxDB]*db.DB, txCache interface{}) (*subscribeReqXlator, error) {
	...
	if subReq.ygPath, _, err = XfmrRemoveXPATHPredicates(reqUri); err != nil {
		log.V(lvl.WARNING).Infof("%v Received error: %v from the XfmrRemoveXPATHPredicates function: ", subReq.reqLogId, err)
		return nil, err
	}

	if subReq.gPath, err = ygot.StringToPath(reqUri, ygot.StructuredPath); err != nil {
		log.V(lvl.WARNING).Infof("%v Error in converting the URI into GNMI path for the URI: %v", subReq.reqLogId, reqUri)
...
func (reqXlator *subscribeReqXlator) processSubscribePath() error {
	...
	for {
		...
		if !ok || ygXpathInfoTmp == nil || ygEntryTmp == nil {
			log.V(lvl.WARNING).Info(reqXlator.subReq.reqLogId, "processSubscribePath: xYangSpecMap does not have the yangXpathInfo for the path:", ygPathTmp)
			return tlerr.InternalError{Format: "xYangSpecMap does not have the yangXpathInfo", Path: ygPathTmp}
		}
	}
	...
	if !reqXlator.validateYangPath(reqXlator.subReq.reqUri, xPathInfoTrgt) {
		log.V(lvl.WARNING).Infof("%v URI path %v is not valid since validate callback function '%v' returned false for this path: ", reqXlator.subReq.reqLogId, reqXlator.subReq.reqUri, xPathInfoTrgt.validateFunc)
...
func (reqXlator *subscribeReqXlator) Translate(targetOnly bool) error {
	...
	if reqXlator == nil {
		log.V(lvl.WARNING).Infof("Translate: subscribeReqXlator is nil")
		return tlerr.InternalError{Format: "subscribeReqXlator is nil"}
	}

	if log.V(dbLgLvl) {
		log.Infof("%v Translate: reqXlator: %v", reqXlator.subReq.reqLogId, *reqXlator.subReq)
	}

	var err error
	ygXpathInfoTrgt, ok := xYangSpecMap[reqXlator.subReq.ygPath]
	if !ok || ygXpathInfoTrgt == nil {
		log.V(lvl.WARNING).Infof("%v Translate: ygXpathInfo data not found in the xYangSpecMap for xpath : %v", reqXlator.subReq.reqLogId, reqXlator.subReq.ygPath)
	...
	if err = reqXlator.processSubscribePath(); err != nil {
		log.V(lvl.WARNING).Infof("%v Translate: Error in processSubscribePath for the path: %v, and the error is %v", reqXlator.subReq.reqLogId, reqXlator.subReq.reqUri, err)
	...
	if err = reqXlator.translateTargetNodePath(ygXpathInfoTrgt); err != nil {
		log.V(lvl.WARNING).Info(reqXlator.subReq.reqLogId, "Translate: Error in translating the target node subscribe path: ", err)
		return err
	}
	if !targetOnly {
		if err = reqXlator.translateChildNodePaths(ygXpathInfoTrgt); err != nil {
			log.V(lvl.WARNING).Info(reqXlator.subReq.reqLogId, "Translate: Error in translating the child node for the subscribe path: ", err)
			return err
		}
	}
...
func (reqXlator *subscribeReqXlator) translateTargetNodePath(trgtYgxPath *yangXpathInfo) error {
	...
	if err != nil {
		log.V(lvl.WARNING).Info(reqXlator.subReq.reqLogId, "Error in getSubscribePathXlator: error: ", err)
		return err
	}
	subMode := trgtPathXlator.getSubscribeMode(nil)
	if trgtPathXlator.pathXlateInfo.OnChange == OnchangeDisable && subMode == OnChange {
		log.V(lvl.WARNING).Infof("%v translateTargetNodePath:Subscribe not supported; on change disabled for the given subscribe path:: %v", reqXlator.subReq.reqLogId, reqXlator.subReq.reqUri)
	...
	if len(tblFld) > 0 {
		...
		if cmpCntVal, ok := tblFld[KEY_COMP_CNT]; ok {
			...
			if keyCmpCnt, err = strconv.Atoi(cmpCntVal); err != nil {
				log.V(lvl.WARNING).Info(pathXltr.subReq.reqLogId, "handleSubtreeNodeXlate:getKeyCompCnt: Invalid value: ", cmpCntVal, " mentioned for the attribute: ", KEY_COMP_CNT, " in the subscribe function: ", ygXpathInfo.xfmrFunc)
...
func (pathXltr *subscribePathXlator) handleSubtreeNodeXlate() error {
	...
	subInParam := XfmrSubscInParams{uriSubtree, pathXltr.uriPath, pathXltr.subReq.dbs, make(RedisDbMap), TRANSLATE_SUBSCRIBE}
	...
	if !subOutPram.isVirtualTbl && pathXltr.pathXlateInfo.OnChange == OnchangeDisable && subNotfMode == OnChange {
		log.V(lvl.WARNING).Info(pathXltr.subReq.reqLogId, "handleSubtreeNodeXlate: Onchange subscription is not supported; onChange flag set to"+
	...
	if subOutPram.isVirtualTbl {
		log.V(dbLgLvl).Info(pathXltr.subReq.reqLogId, "handleSubtreeNodeXlate: isVirtualTbl flag is set to true for the path: ", pathXltr.uriPath)
		return nil
	}
	...
	for dbNum, tblKeyInfo := range subOutPram.secDbDataMap {
		...
		if dbNum == db.CountersDB && (pathXltr.subReq.subReqMode == OnChange && pathXltr.pathXlateInfo.OnChange != OnchangeEnable) {
			log.V(lvl.WARNING).Info(pathXltr.subReq.reqLogId, "handleSubtreeNodeXlate: Onchange subscription is not supported for COUNTERS_DB by default for the path: ", pathXltr.uriPath)
		...
		for tblName, tblFieldInfo := range tblKeyInfo {
			...
			for dBKey, nodeIntf := range tblFieldInfo {
				...
				case apis.ProcessOnChange:
					...
					continue
				default:
					log.V(lvl.WARNING).Infof("%s handleSubtreeNodeXlate: Subscribe callback '%v' returned incorrect type: %T",
				...
				for dbField, yangNodeName := range dbFldYgNameMap {
					...
					if ygLeafXpathInfo == nil {
						log.V(lvl.WARNING).Info(pathXltr.subReq.reqLogId, "handleSubtreeNodeXlate: XpathInfo not found for the leaf path: ", ygLeafNodePath)
					...
					if err != nil {
						log.V(lvl.WARNING).Info(pathXltr.subReq.reqLogId, "handleSubtreeNodeXlate: error in StringToPath: err: ", err)
						return err
					}
					if log.V(dbLgLvl) {
			...
			for tblName, tblFieldInfo := range tblKeyInfo {
				...
				for dBKey, tblFld := range tblFieldInfo {
					...
					if fldNamePattern, ok := tblFld[FIELD_CURSOR]; ok {
						...
						log.V(dbLgLvl).Infof("%v handleSubtreeNodeXlate: fldNamePattern %v", pathXltr.subReq.reqLogId, fldNamePattern)
					...
					for dbFld, ygLeafNames := range tblFld {
						...
						for _, ygNodeName := range nodeNames {
							...
							if strings.HasPrefix(nodeName, "{") {
								...
								log.V(dbLgLvl).Info(pathXltr.subReq.reqLogId, "handleSubtreeNodeXlate: dbFld map - yang node after trimming: ", nodeName)
...
func (pathXltr *subscribePathXlator) addDbFldYangMapInfo() error {
	...
	if len(pathXltr.pathXlateInfo.ygXpathInfo.compositeFields) > 0 {
		...
		for _, dbTblFldName := range pathXltr.pathXlateInfo.ygXpathInfo.compositeFields {
			...
			if len(tblField) <= 1 {
				log.V(lvl.WARNING).Info(pathXltr.subReq.reqLogId, "subscribePathXlator: addDbFldYangMapInfo: Table name is missing in the composite-db-fields annoation for the leaf node path:", pathXltr.uriPath)
			...
			/* Google: This check was moved outside of the for loop baceause we only expect the table name to map to one or more table in the composite fields annotation.
			if dbKeyInfo == nil || len(dbKeyInfo.DbFldYgMapList) == 0 {
				log.V(lvl.WARNING).Infof("%v subscribePathXlator: addDbFldYangMapInfo: Table name %v is not mapped to this path:", pathXltr.subReq.reqLogId, tblName, pathXltr.uriPath)
		...
		if !tblMapped {
			log.V(lvl.WARNING).Infof("%v subscribePathXlator: addDbFldYangMapInfo: Table name is not mapped for this path: %v", pathXltr.subReq.reqLogId, pathXltr.uriPath)
			return tlerr.InternalError{Format: "Table name is not mapped for this leaf node path", Path: pathXltr.uriPath}
		}
...
func (pathXltr *subscribePathXlator) translatePath() error {
	...
	if pathXltr.subReq.xlateNodeType == TARGET_NODE {
		...
		log.V(lvl.WARNING).Infof("%v translatePath: pathXltr.xpathYgNode not found for the path: %v", pathXltr.subReq.reqLogId, pathXltr.uriPath)
	}
	...
	if len(ygXpathInfoTrgt.xfmrFunc) > 0 {
		...
		if pathXltr.subReq.subReqMode == OnChange && (ygXpathInfoTrgt.dbIndex == db.CountersDB && !ygXpathInfoTrgt.subscriptionFlags.Has(subsOnChangeEnable)) {
			log.V(lvl.WARNING).Infof("%v translatePath:Subscribe not supported for COUNTERS_DB by default for the path:: %v", pathXltr.subReq.reqLogId, pathXltr.uriPath)
...
func (pathXltr *subscribePathXlator) handleYangToDbKeyXfmr() (string, error) {
	...
	if pathXltr.keyXfmrYgXpathInfo == nil {
		log.V(lvl.WARNING).Info(pathXltr.subReq.reqLogId, "handleYangToDbKeyXfmr: yangXpathInfo is nil in the xYangSpecMap for the path: ", pathXltr.uriPath)
	...
	if len(pathXltr.keyXfmrYgXpathInfo.xfmrKey) > 0 {
		...
		if err != nil {
			log.V(lvl.WARNING).Info(pathXltr.subReq.reqLogId, "Error: newYgotRootObj error: ", err)
			return "", err
		}
		...
		if errKey == nil {
			...
			log.V(lvl.WARNING).Info(pathXltr.subReq.reqLogId, "handleYangToDbKeyXfmr: keyXfmrHandler callback error:", errKey)
		}
		...
		log.V(dbLgLvl).Info(pathXltr.subReq.reqLogId, "handleYangToDbKeyXfmr: keyDelm: ", keyDelm)
		...
		for idx := 1; idx < len(pathElems); idx++ {
			...
			if len(pathElems[idx].Key) > 0 {
				if ygXpathInfo, ok := xYangSpecMap[ygPath]; ok {
					...
					log.V(lvl.WARNING).Info(pathXltr.subReq.reqLogId, "handleYangToDbKeyXfmr: xpathinfo not found for the ygpath: ", ygPath)
				}
			}
		}
		if log.V(dbLgLvl) {
...
func (pathXltr *subscribePathXlator) handleNonSubtreeNodeXlate() error {
	...
	for tblName := range tblNameMap {
		...
		if log.V(dbLgLvl) {
			log.Infof("%v handleNonSubtreeNodeXlate: Alias mode; resolving the db key %v for the tablename: %v", pathXltr.subReq.reqLogId, dbKey, tblName)
		}
...
func (pathXltr *subscribePathXlator) handleTableXfmrCallback() ([]string, error) {
	...
	if err != nil {
		log.V(lvl.WARNING).Infof(pathXltr.subReq.reqLogId, "newYgotRootObj: error: %v", err)
		return nil, err
	}
...
func (reqXlator *subscribeReqXlator) translateChildNodePaths(ygXpathInfo *yangXpathInfo) error {
	...
	if err = reqXlator.collectChldYgXPathInfo(ygNode, reqXlator.subReq.ygPath, rltvUriPath, ygXpathInfo, reqXlator.trgtYgNode); err != nil {
		log.V(lvl.WARNING).Info(reqXlator.subReq.reqLogId, "translateChildNodePaths: Error in collectChldYgXPathInfo; error: ", err)
		return err
	}
...
func (reqXlator *subscribeReqXlator) traverseYgXpathAndTranslate(ygXpNode *ygXpathNode, parentRelUri string, parentPathXlateInfo *XfmrSubscribePathXlateInfo) error {
	log.V(dbLgLvl).Infof("%v traverseYgXpathAndTranslate: ygXpNode path:%v; parentRelUri: %v; parentPathXlateInfo path: %v ",
	...
	for _, chldNode := range ygXpNode.chldNodes {
		...
		if chldNode.isParentTbl {
			...
			if err != nil && reqXlator.subReq.subReqMode == Sample {
				log.V(lvl.WARNING).Infof("%v traverseYgXpathAndTranslate: Skip child node '%s' due to error in translatePath: %v",
					reqXlator.subReq.reqLogId, uriPath, err)
				continue
			}
			if err != nil {
				log.V(lvl.WARNING).Infof("%v traverseYgXpathAndTranslate: Error in translatePath: %v for the path %v", reqXlator.subReq.reqLogId, err, uriPath)
				return err
			}
			...
			if !forceSeparateEntry && chldNode.pathXlateInfo.isSamePathXlateInfo(parentPathXlateInfo) {
				...
				if !hasDbTblInfo {
					...
					log.V(lvl.WARNING).Infof("%s traverseYgXpathAndTranslate: Db table information is not found for the node path : %v; traversing the child nodes with grand parent", reqXlator.subReq.reqLogId, uriPath)
					pathXlateInfo = parentPathXlateInfo
				}
			}
...
func (pathXlateInfo *XfmrSubscribePathXlateInfo) copyDbFldYgPathMap(parentRelUri string, ygXpNode *ygXpathNode) {
	...
	if sIdx := strings.Index(ygXpNode.relUriPath, parentRelUri); sIdx == -1 {
		log.V(lvl.WARNING).Info(pathXlateInfo.reqLogId, "copyDbFldYgPathMap: Not able to get the relative path of the node for the relUriPath: ", ygXpNode.relUriPath)
	} else {
		if log.V(dbLgLvl) {
...
func (pathXlateInfo *XfmrSubscribePathXlateInfo) addDbFldYgPathMap(relPath string, ygXpNode *ygXpathNode) error {
	...
	if len(pathXlateInfo.DbKeyXlateInfo) == 0 && len(ygXpNode.dbFldYgPathMap) > 0 {
		log.V(lvl.WARNING).Info(pathXlateInfo.reqLogId, "addDbFldYgPathMap: pathXlateInfo.DbKeyXlateInfo is empty for the path ", ygXpNode.ygPath)
	...
	if len(ygXpNode.dbTblFldYgPathMap) > 0 {
		...
		for _, dbKeyInfo := range pathXlateInfo.DbKeyXlateInfo {
			if dbFldYgMap, ok := ygXpNode.dbTblFldYgPathMap[dbKeyInfo.Table.Name]; ok {
				...
				log.V(lvl.WARNING).Infof("%v addDbFldYgPathMap: Not able to find the db field mapping of the node: %v for the table %v", pathXlateInfo.reqLogId, ygXpNode.ygPath, dbKeyInfo.Table.Name)
...
func (ygXpNode *ygXpathNode) addDbFldNames(ygNodeName string, dbFldNames []string) error {
	for _, dbTblFldName := range dbFldNames {
		tblField := strings.Split(dbTblFldName, ":")
		if len(tblField) <= 1 {
			log.V(lvl.WARNING).Info("addDbFldNames: Table name is missing in the composite-db-fields annoation for the leaf node path:", ygXpNode.relUriPath, "/", ygNodeName)
...
func (reqXlator *subscribeReqXlator) collectChldYgXPathInfo(ygEntry *yang.Entry, ygPath string,
	...
	log.V(dbLgLvl).Infof("%v collectChldYgXPathInfo: ygEntry: %v, ygPath: %v, rltvUriPath: %v; table name: %v; parent node path: %v",
		...
		if chYgXpathInfo, ok := xYangSpecMap[childYgPath]; ok {
			...
			if childYgEntry.IsList() {
				...
				if !reqXlator.validateYangPath(chldPathUri, chYgXpathInfo) {
					log.V(lvl.WARNING).Infof("%v URI path %v is not valid since validate callback function '%v' returned false",
			...
			if chYgXpathInfo.dbIndex == db.CountersDB && !chYgXpathInfo.subscriptionFlags.Has(subsOnChangeEnable) && reqXlator.subReq.subReqMode == OnChange {
				log.V(lvl.WARNING).Info(reqXlator.subReq.reqLogId, "CountersDB mapped in the path: ", childYgPath)
			...
			if childYgEntry.IsLeaf() || childYgEntry.IsLeafList() {
				...
				if err := reqXlator.collectChldYgXPathInfo(childYgEntry, childYgPath, rltvChldUriPath, chYgXpathInfo, chldNode); err != nil {
					log.V(lvl.WARNING).Info(reqXlator.subReq.reqLogId, "Error in collecting the ygXpath Info for the yang path: ", childYgPath, " and the error: ", err)
					return err
				}
			}
		} else if childYgEntry.IsList() || childYgEntry.IsContainer() {
...
func (reqXlator *subscribeReqXlator) uriToAbsolutePath(rltvUri string) (*gnmipb.Path, error) {
	...
	if err != nil {
		log.V(lvl.WARNING).Info(reqXlator.subReq.reqLogId, "Error in converting the URI into GNMI path for the URI: ", rltvUri)
...
func debugPrintXPathInfo(xpathInfo *yangXpathInfo) {
	log.V(lvl.INFO).Infof("    yangType: %v\r\n", getYangTypeStrId(xpathInfo.yangType))
	log.V(lvl.INFO).Info("      fieldName: ", xpathInfo.fieldName)
	if xpathInfo.nameWithMod != nil {
		log.V(lvl.INFO).Infof("    nameWithMod : %v\r\n", *xpathInfo.nameWithMod)
	} else {
		log.V(lvl.INFO).Info("      nameWithMod: ", xpathInfo.nameWithMod)
	}
	log.V(lvl.INFO).Infof("    hasChildSubTree : %v\r\n", xpathInfo.hasChildSubTree)
	log.V(lvl.INFO).Infof("    hasNonTerminalNode : %v\r\n", xpathInfo.hasNonTerminalNode)
	log.V(lvl.INFO).Infof("    subscribeOnChg Disable  : %v\r\n", xpathInfo.subscriptionFlags.Has(subsOnChangeDisable))
	log.V(lvl.INFO).Infof("    subscribeMinIntvl  : %v\r\n", xpathInfo.subscribeMinIntvl)
	log.V(lvl.INFO).Infof("    subscribePref Sample  : %v\r\n", xpathInfo.subscriptionFlags.Has(subsPrefSample))

	log.V(lvl.INFO).Infof("    tableName: ")
	if xpathInfo.tableName != nil {
		log.V(lvl.INFO).Infof("%v", *xpathInfo.tableName)
	} else {
		log.V(lvl.INFO).Infof("%v", xpathInfo.tableName)
	}
	log.V(lvl.INFO).Infof("\r\n    virtualTbl: ")
	if xpathInfo.virtualTbl != nil {
		log.V(lvl.INFO).Infof("%v", *xpathInfo.virtualTbl)
	} else {
		log.V(lvl.INFO).Infof("%v", xpathInfo.virtualTbl)
	}
	log.V(lvl.INFO).Infof("\r\n    xfmrTbl  : ")
	if xpathInfo.xfmrTbl != nil {
		log.V(lvl.INFO).Infof("%v", *xpathInfo.xfmrTbl)
	} else {
		log.V(lvl.INFO).Infof("%v", xpathInfo.xfmrTbl)
	}
	log.V(lvl.INFO).Infof("\r\n    keyName  : ")
	if xpathInfo.keyName != nil {
		log.V(lvl.INFO).Infof("%v", *xpathInfo.keyName)
	} else {
		log.V(lvl.INFO).Infof("%v", xpathInfo.keyName)
	}
	...
	if xpathInfo.yangEntry != nil {
		log.V(lvl.INFO).Infof("%v", *xpathInfo.yangEntry)
	} else {
		log.V(lvl.INFO).Infof("%v", xpathInfo.yangEntry)
	}
	log.V(lvl.INFO).Infof("\r\n    keyXpath: %d\r\n", xpathInfo.keyXpath)
	for i, kd := range xpathInfo.keyXpath {
		log.V(lvl.INFO).Infof("        %d : xpathInfo. %#v\r\n", i, kd)
	}
	log.V(lvl.INFO).Infof("\r\n    isKey   : %v\r\n", xpathInfo.isKey)
	log.V(lvl.INFO).Info("      delim: ", xpathInfo.delim)
}
...
func getYgEntry(reqLogId string, ygXpath *yangXpathInfo, ygPath string) (*yang.Entry, error) {
	...
	if ygEntry == nil {
		if log.V(dbLgLvl) {
			log.Warningf("%v : yangEntry is nil in the yangXpathInfo for the path: %v", reqLogId, ygPath)
		}
...
func (keyRslvr *DbYangKeyResolver) resolveDbKey(keyList []string, operation Operation) ([]string, error) {
	...
	for idx := 0; idx < len(keyList); idx++ {
		...
		if !ok || keyYgNode == nil {
			log.V(lvl.WARNING).Infof("%v resolveDbKey: Db yang node not found in the xDbSpecMap for the path: %v", keyRslvr.reqLogId, keyPath)
			continue
		}
		if log.V(dbLgLvl) {
		...
		if keyYgNode.xfmrValue != nil && len(*keyYgNode.xfmrValue) > 0 {
			...
			for _, leafRefPath := range keyYgNode.leafRefPath {
				...
				if !ok || keyLeafRefNode == nil {
					log.V(lvl.WARNING).Infof("%v resolveDbKey: Db yang key leafref node not found in the xDbSpecMap for the path: ", keyRslvr.reqLogId, leafRefPath)
					continue
				}
				if keyLeafRefNode.xfmrValue != nil && len(*keyLeafRefNode.xfmrValue) > 0 {
					...
					if len(keyLeafVal) == 0 || err != nil {
						log.V(lvl.WARNING).Infof("%v resolveDbKey: valueXfmrHandler error: %v; given keyPath: %v; keyLeafVal: %v for the "+
							"uri path %v", keyRslvr.reqLogId, err, keyPath, keyLeafVal, keyRslvr.uriPath)
						continue
					} else {
						break
	...
	log.V(lvl.WARNING).Infof("%v resolveDbKey: Could not resolve the db key %v; yang schema not found for the table %v "+
...
func (keyRslvr *DbYangKeyResolver) resolve(operation Operation) ([]string, error) {
	...
	for _, listName := range ygDbInfo.listName {
		...
		if !ygDbListNode.IsList() {
			log.V(lvl.WARNING).Infof("%v DbYangKeyResolver: resolve: list name %v is not found in the xDbSpecMap as yang list node for the table: %v", keyRslvr.reqLogId, listName, keyRslvr.tableName)
			continue
		}
		...
		if len(keyList) != keyRslvr.key.Len() {
			log.V(lvl.WARNING).Infof("%v DbYangKeyResolver: db table %v key count %v and yang model list %v key count %v does not match",
	...
	log.V(lvl.WARNING).Infof("%v DbYangKeyResolver: Db yang matching list node not found for the table %v for the path: %v", keyRslvr.reqLogId, keyRslvr.tableName, keyRslvr.uriPath)
	return keyRslvr.key.Comp, nil
}
