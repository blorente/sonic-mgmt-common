	...
	if !isFirstCall {
		...
		if err != nil {
			...
			case tlerr.TranslibXfmrRetError:
				...
				log.V(lvl.WARNING).Infof("Error received (\"%v\"), ecode :%v", err, ecode)
				if ecode {
					return err
				}
			}
					...
					if isFirstCall {
						if perr == nil && cerr == nil {
							if len(curTbl) > 0 && parentTbl != curTbl {
								...
								log.V(lvl.WARNING).Infof("No table found for Uri - %v ", curUri)
							}
						}
					} else {
...
func yangContainerDelData(xlateParams xlateToParams, dbDataMap *map[db.DBNum]map[string]map[string]db.Value, subTreeResMap *map[string]map[string]db.Value, isFirstCall bool) error {
	...
	if len(spec.xfmrFunc) == 0 {
		...
		curTbl = xpathKeyExtRet.tableName
		if cerr != nil {
			log.V(lvl.WARNING).Infof("Received xpathKeyExtract error for uri: %v : err %v", xlateParams.uri, cerr)
			switch e := err.(type) {
			case tlerr.TranslibXfmrRetError:
				ecode := e.XlateFailDelReq
		...
		if ok {
			...
			if len(spec.xfmrFunc) > 0 {
				...
				if len(xpathKeyExtRet.tableName) > 0 && len(xpathKeyExtRet.dbKey) > 0 {
					...
					log.V(lvl.WARNING).Infof("No proper table and key information to fill result map for URI %v, table: %v, key %v", uri, xpathKeyExtRet.tableName, xpathKeyExtRet.dbKey)
				}
			} else {
					...
					if ok && dbEntry != nil {
						...
						if yangType == YANG_LEAF_LIST {
							...
							if valErr == nil {
								...
								if err != nil {
									log.V(lvl.WARNING).Infof("Couldn't unmarshal Json to DbData: path(\"%v\") error (\"%v\").", dbSpecField, err)
									return err
								}
							}
							fieldName = fieldName + "@"
