...
func sortPerTblDeps(ordTblListMap map[string][]string) error {
	...
	if status != nil {
		log.V(lvl.WARNING).Infof("CVL validation session creation failed(%v).", status)
	...
	for tname, tblList := range ordTblListMap {
		...
		if status != cvl.CVL_SUCCESS {
			log.V(lvl.WARNING).Infof("Failure in cvlSess.SortDepTables: %v", status)
...
func xlateJsonTblInfoLoad(ordTblListMap map[string][]string, jsonFileName string) error {
	...
	for i := 0; i < len(tlist.TblInfo); i++ {
		...
		if err != nil {
			log.V(lvl.WARNING).Infof("Failed to add table dependency(tbl:%v, par:%v) into tablenode list.(%v)\r\n",
