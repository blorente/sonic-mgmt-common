...
func init() {
	...
	log.V(lvl.INFO).Info("xspec init done ...")
}
...
func addMdlCpbltEntry(yangMdlNm string) {
	...
	if mdlInfoEntry == nil {
		log.V(lvl.WARNING).Infof("Memory allocation failure for storing model info for gnmi - module %v", yangMdlNm)
		return
	}
	mdlInfoEntry.Org = ""
	mdlInfoEntry.Ver = ""
...
func yangToDbMapFill(keyLevel uint8, xYangSpecMap map[string]*yangXpathInfo, entry *yang.Entry, xpathPrefix string, xpathFull string) {
	...
	if entry != nil && (entry.Kind == yang.CaseEntry || entry.Kind == yang.ChoiceEntry) {
		...
		if ok && (parentXpathData.subscribeMinIntvl == XFMR_INVALID) {
			log.V(lvl.WARNING).Infof("Susbscribe MinInterval/OnChange flag is set to invalid for(%v) \r\n", xpathPrefix)
			return
		}
		...
		if ok {
			...
			if parentXpathData.cascadeDel == XFMR_INVALID {
				...
				log.V(lvl.WARNING).Infof("Cascade-delete flag is set to invalid for(%v) \r\n", xpathPrefix)
				return
			}
...
func dbSpecXpathGet(inPath string) (string, error) {
	...
	if err != nil {
		log.V(lvl.WARNING).Infof("xpath conversion failed for(%v) \r\n", inPath)
		return specPath, err
	}

	pathList := strings.Split(xpath, "/")
	if len(pathList) < 3 {
		log.V(lvl.WARNING).Infof("Leaf-ref path not valid(%v) \r\n", inPath)
		return specPath, err
	}
...
func dbMapFill(tableName string, curPath string, moduleNm string, xDbSpecMap map[string]*dbInfo, entry *yang.Entry) {
	...
	if entry.Name != moduleNm {
		...
		if tableContainer {
			...
			if entry.IsList() && entry.Parent != nil && entry.Parent.IsList() { // nested/child list of list under table-level container
				for siblingNm := range entry.Parent.Dir {
					...
					log.V(lvl.WARNING).Infof("Nested list %v can have only key-leaf siblings under parent list %v for table %v in module %v",
				...
				log.V(lvl.WARNING).Infof("Nested list %v not supported under a non-table level container yang node %v in module %v", entry.Name, entry.Parent.Name, moduleNm)
				return
			}
		...
		if tableContainer {
			...
			if entry.Exts != nil && len(entry.Exts) > 0 {
				for _, ext := range entry.Exts {
					...
					default:
						log.V(lvl.INFO).Infof("Unsupported ext type(%v) for xpath(%v).", tagType, dbXpath)
					}
				}
			}
			...
			if entry.Type.Kind == yang.Yleafref {
				...
				if strings.Contains(lrefpath, "..") {
					...
					if lerr != nil {
						log.V(lvl.WARNING).Infof("Failed to add leaf-ref for(%v) \r\n", entry.Type.Path)
						return
					}
		...
		for {
			...
			if sncTblInfo == nil {
				log.V(lvl.WARNING).Infof("Memory allocation failure for storing Tbl order and dependency info for sonic module %v", moduleNm)
				break
			}
			cvlSess, cvlRetSess := db.NewValidationSession()
			if cvlRetSess != nil {
				log.V(lvl.WARNING).Infof("Failure in creating CVL validation session object required to use CVl API to get Tbl info for module %v - %v", moduleNm, cvlRetSess)
				break
			}
			...
			if cvlRetOrdTbl != cvl.CVL_SUCCESS {
				log.V(lvl.WARNING).Infof("Failure in cvlSess.GetOrderedTables(%v) - %v", moduleNm, cvlRetOrdTbl)

			}
			sncTblInfo.DepTbl = make(map[string]depTblData)
			if sncTblInfo.DepTbl == nil {
				log.V(lvl.WARNING).Infof("sncTblInfo.DepTbl is nill , no space to store dependency table list for sonic module %v", moduleNm)
			...
			for _, tbl := range sncTblInfo.OrdTbl {
				...
				if cvlRetDepTbl != cvl.CVL_SUCCESS {
					log.V(lvl.WARNING).Infof("Failure in cvlSess.GetOrderedDepTables(%v, %v) - %v", moduleNm, tbl, cvlRetDepTbl)
				}
				depTblInfo.DepTblAcrossMdl, cvlRetDepTbl = cvlSess.GetDepTables(moduleNm, tbl)

				if cvlRetDepTbl != cvl.CVL_SUCCESS {
					log.V(lvl.WARNING).Infof("Failure in cvlSess.GetDepTables(%v, %v) - %v", moduleNm, tbl, cvlRetDepTbl)
				}
		...
		for _, ext := range entry.Exts {
			...
			case "subscribe-on-change":
				if strings.EqualFold(ext.NName(), "disable") {
					...
					log.V(lvl.WARNING).Infof("Invalid subscribe-on-change value: %v defined in the path %v\r\n", ext.NName(), xpath)
				}
			case "subscribe-min-interval":
				if ext.NName() == "NONE" {
					...
					if err != nil {
						log.V(lvl.WARNING).Infof("Invalid subscribe min interval time(%v).\r\n", ext.NName())
						return
					}
			...
			case "db-key-count":
				...
				if xpathData.dbKeyCompCnt, err = strconv.Atoi(ext.NName()); err != nil {
					log.V(lvl.WARNING).Infof("Invalid db-key-count value (%v) in the yang path %v.\r\n", ext.NName(), xpath)
					return
				}
...
func annotDbSpecMapFill(xDbSpecMap map[string]*dbInfo, dbXpath string, entry *yang.Entry) error {
	...
	if len(pname) < 3 {
		...
		if entry != nil && len(entry.Exts) > 0 {
			...
			log.V(lvl.WARNING).Infof("DB Rpc spec-map doesn't contain rpc entry(%v) \r\n", dbXpath)
		}
		return err
	}
	...
	if !ok {
		log.V(lvl.WARNING).Infof("DB spec-map data not found(%v) \r\n", dbXpath)
		return err
	}
...
func sonicYangNestedListValidateElements(tableName string, entry *yang.Entry) error {
	...
	*/
	...
	log.V(lvl.WARNING).Infof(errStr)
