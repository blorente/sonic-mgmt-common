...
func DbValToInt(dbFldVal string, base int, size int, isUint bool) (interface{}, error) {
	...
	if isUint {
		if res, err = strconv.ParseUint(dbFldVal, base, size); err != nil {
			log.V(lvl.WARNING).Infof("Non Yint%v type for YANG leaf-list item %v", size, dbFldVal)
		}
	} else {
...
func DbToYangType(yngTerminalNdDtType yang.TypeKind, fldXpath string, dbFldVal string, oper Operation) (interface{}, interface{}, error) {
	...
	case yang.Ynone:
		log.V(lvl.WARNING).Info("Yang node data-type is non base YANG type")
	...
	case yang.Ybool:
		if res, err = strconv.ParseBool(dbFldVal); err != nil {
			log.V(lvl.WARNING).Infof("Non Bool type for YANG leaf-list item %v", dbFldVal)
		}
	...
	default:
		for _, fldVal := range valLst {
			...
			if err == nil {
				...
				log.V(lvl.WARNING).Infof("Failed to convert DB value type to YANG type for xpath %v. Field xfmr recommended if data types differ", fieldXpath)
			}
		}
	}
	return resLst
...
func sonicDbToYangTerminalNodeFill(field string, inParamsForGet xlateFromDbParams, dbEntry *yang.Entry, isNestedListEntry bool, isKeyLeaf bool) {
	...
	if dbEntry == nil {
		log.V(lvl.WARNING).Infof("Yang entry is nil for xpath %v", inParamsForGet.xpath)
		return
	}
...
func sonicDbToYangListFill(inParamsForGet xlateFromDbParams) ([]typeMapOfInterface, error) {
	...
	if isReqContextCancelled(inParamsForGet.reqCtxt) {
		...
		log.V(lvl.WARNING).Infof(err.Error())
		return mapSlice, err
	}
...
func sonicDbToYangNestedListDataFill(inParamsForGet xlateFromDbParams) ([]typeMapOfInterface, error) {
	...
	for field, value := range dbTblData.Field {
		...
		if err != nil {
			log.V(lvl.WARNING).Info("Conversion of DB value type to YANG type for field didn't happen. Field xpath", keyLeafXpath)
		...
		if !skipNonKeyLeafAdd {
			...
			if err != nil {
				log.V(lvl.WARNING).Info("Conversion of DB value type to YANG type for field didn't happen. Field xpath", nonKeyLeafXpath)
...
func directDbToYangJsonCreate(inParamsForGet xlateFromDbParams) (string, bool, error) {
	...
	if isEmptyPayload {
		log.V(lvl.WARNING).Info("No data available")
	}
...
func fillDbDataMapForTbl(uri string, xpath string, tblName string, tblKey string, cdb db.DBNum, dbs [db.MaxDB]*db.DB,
	...
	if tblKey != "" {
		...
		if tblSpecInfo, ok := xDbSpecMap[tblName]; ok && tblSpecInfo.hasXfmrFn {
			...
			if err != nil {
				log.V(lvl.WARNING).Infof("Value-xfmr for table(%v) & key(%v) didn't do conversion.", tblName, tblKey)
				return nil, err
			}
		}
...
func yangListDataFill(inParamsForGet xlateFromDbParams, isFirstCall bool, isOcMdl bool) error {
	...
	if len(parentUriPath) > 0 {
		...
		if err != nil {
			log.V(lvl.WARNING).Infof("yangListDataFill: error in unmarshaling the parent uri: %v; error: %v; "+
	...
	if len(tblList) == 0 {
		if (strings.HasSuffix(uri, "]") || strings.HasSuffix(uri, "]/")) && (len(xYangSpecMap[xpath].xfmrFunc) > 0 && xYangSpecMap[xpath].hasChildSubTree) {
			...
			if err != nil {
				xfmrLogInfo("yangListDataFill: error in its child subtree traversal for the xpath: %v error: %v", xpath, err)
				_, qpSbtPruneErrOk := err.(*qpSubtreePruningErr)
				if qpSbtPruneErrOk {
					return err
				}
	...
	if ok && len(xYangSpecMap[xpath].xfmrFunc) > 0 {
		if isFirstCall || (!isFirstCall && (uri != requestUri) && ((len(xYangSpecMap[parentXpath].xfmrFunc) == 0) ||
			...
			if err != nil {
				...
				if ygotCtx.err != nil {
					log.V(lvl.WARNING).Infof("yangListInstanceDataFill: error in unmarshalling the URI: %v, relUri: %v, "+
		...
		if uri == requestUri {
			...
			if len(fldValMap) == 0 {
				...
				if (yangType == YANG_LEAF) || ((yangType == YANG_LEAF_LIST) && ((strings.HasSuffix(uri, "]")) || (strings.HasSuffix(uri, "]/")))) {
					log.V(lvl.WARNING).Infof("Field transformer returned empty data , URI  - %v", requestUri)
		...
		if (terminalNodeQuery || dbKeyExist) && xYangSpecMap[xpath].isKey { //GET request for list key-leaf(direct child of list)
			...
			if yangType == YANG_LEAF_LIST {
				...
				if ok {
					if leafLstInstGetReq {
						...
						if dbSpecOk && dbSpecFieldInfo.xfmrValue != nil {
							...
							if valXfmrErr != nil {
								log.V(lvl.WARNING).Infof("value-xfmr:fldpath(\"%v\") val(\"%v\"):err(\"%v\").", dbSpecField, leafListInstVal, valXfmrErr)
						...
						if !leafListInstExists((*dbDataMap)[cdb][tbl][tblKey].Field[dbFldName], leafListInstVal) {
							log.V(lvl.WARNING).Infof("Queried leaf-list instance does not exists, URI  - %v, dbData - %v", requestUri, (*dbDataMap)[cdb][tbl][tblKey].Field[dbFldName])
							err = tlerr.NotFoundError{Format: "Resource not found"}
						}
						if err == nil {
							...
							*/
							log.V(lvl.INFO).Info("Queried leaf-list instance exists but Since translib already fills in ygRoot with queried leaf-list instance do not populate payload.")
						}
						return resFldValMap, err
					} else {
					...
					if leafLstInstGetReq {
						log.V(lvl.WARNING).Infof("Queried leaf-list does not exist in DB, URI  - %v", requestUri)
						err = tlerr.NotFoundError{Format: "Resource not found"}
					}
				}
			} else {
				...
				if ok {
					...
					if err != nil {
						log.V(lvl.WARNING).Info("Conversion of DB value type to YANG type for field didn't happen. Field-xfmr recommended if data types differ. Field xpath", xpath)
					} else {
						resFldValMap = make(map[string]interface{})
...
func yangDataFill(inParamsForGet xlateFromDbParams, isOcMdl bool) error {
	...
	if ok && yangNode.yangEntry != nil {
		if inParamsForGet.queryParams.depthEnabled {
			...
			log.V(lvl.INFO).Infof("yangDataFill curdepth: %v, Path : %v", inParamsForGet.queryParams.curDepth, xpath)
		...
		if len(inParamsForGet.relUri) > 0 {
			...
			err := ygotXlator{&ygotCtx}.translate()
			if err != nil {
				log.V(lvl.WARNING).Infof("error in unmarshalling the uri: %v; error: %v; parent obj: %v; schema: %v",
			...
			if xYangSpecMap[chldXpath] != nil && yangNode.yangEntry.Dir[yangChldName] != nil {
				...
				if chldYangType == YANG_LEAF || chldYangType == YANG_LEAF_LIST {
					...
					if len(fldValMap) > 0 {
						...
						if errJs != nil {
							log.V(lvl.WARNING).Infof("yangDataFill: Failed to unmarshall json data: %v for the uri : %v; error: %v", jsonDataTmp, chldUri, errJs)
						} else {
					...
					if len(fldValMap) > 0 {
						if err := ytypes.Unmarshal(inParamsForGet.ygSchema, *inParamsForGet.ygParentObj, fldValMap); err != nil {
							log.V(lvl.WARNING).Infof("yangDataFill: error in object unmarshalling: %v; schema: %v; parent obj: %v, "+
								"resFldValMap: %v", err, inParamsForGet.ygSchema.Name, reflect.TypeOf(*inParamsForGet.ygParentObj), fldValMap)
							// Continue processing other children despite this child failing to unmarshall.  This allows us to skip over
							// fields with bad data (but still log a warning) and continue processing other children.
							continue
						}
					}
				} else if chldYangType == YANG_CONTAINER {
					...
					if xYangSpecMap[chldXpath].xfmrTbl != nil {
						...
						if len(xfmrTblFunc) > 0 {
							...
							inParamsForGet.ygRoot = ygRoot
							if len(tblList) > 1 {
								log.V(lvl.WARNING).Infof("Table transformer returned more than one table for container %v", chldXpath)
							}
							if len(tblList) == 0 {
								continue
							}
					...
					if xerr != nil && len(xpathKeyExtRet.dbKey) == 0 {
						log.V(lvl.WARNING).Infof("yangDataFill: could not convert yang key into db key, error: %v; chldUri: %v", xerr.Error(), chldUri)
						continue
					}
					inParamsForGet.ygRoot = ygRoot
					cdb = xYangSpecMap[chldXpath].dbIndex
		...
		log.V(lvl.WARNING).Infof("yangDataFill: xpath info or yang.Entry not found for the xpath: %v; uri: %v", xpath, uri)
	}
	return err
}
			...
			if (!validateHandlerFlag) && (!isList) {
				if xYangSpecMap[xpathKeyExtRet.xpath].xfmrTbl != nil {
					...
					if len(xfmrTblFunc) > 0 {
						...
						inParamsForGet.ygRoot = ygRoot
						if len(tblList) > 1 {
							log.V(lvl.WARNING).Infof("Table transformer returned more than one table for container %v", xpathKeyExtRet.xpath)
						}
						if len(tblList) == 0 {
							log.V(lvl.WARNING).Infof("Table transformer returned no table for conatiner %v", xpathKeyExtRet.xpath)
							tableXfmrFlag = true
						}
						if !tableXfmrFlag {
							for _, tbl := range tblList {
						...
						log.V(lvl.WARNING).Infof("empty table transformer function name for xpath - %v", xpathKeyExtRet.xpath)
						tableXfmrFlag = true
					}
				}
			}
				...
				if yangType == YANG_LEAF || yangType == YANG_LEAF_LIST {
					...
					if len(xYangSpecMap[xpathKeyExtRet.xpath].xfmrFunc) > 0 {
						...
						if len(fldValMap) > 0 {
							...
							errJs := json.Unmarshal(jsonDataTmp, &jsonDataMap)
							if errJs != nil {
								log.V(lvl.WARNING).Infof("dbDataToYangJsonCreate: Failed to unmarshall json data: %v for the uri : %v; error: %v", jsonDataTmp, uri, errJs)
						...
						if len(fldValMap) > 0 {
							...
							if len(parentUriPath) > 0 {
								err := ygotXlator{&ygotCtx}.translate()
								if err != nil {
									log.V(lvl.WARNING).Infof("dbDataToYangJsonCreate: error in unmarshalling the URI: %v, relUri: %v, "+
							...
							if err := ytypes.Unmarshal(inParamsForGet.ygSchema, *inParamsForGet.ygParentObj, fldValMap); err != nil {
								log.V(lvl.WARNING).Infof("dbDataToYangJsonCreate: error in object unmarshalling: %v; schema: %v; parent obj: %v, "+
					...
					err = yangDataFill(inParamsForGet, isOcMdl)
					if err != nil {
						xfmrLogInfo("Empty container(\"%v\") error: %v", uri, err)
					}
					resultMap = inParamsForGet.resultMap
					break
				} else if yangType == YANG_LIST {
					...
					if len(xYangSpecMap[xpathKeyExtRet.xpath].xfmrFunc) > 0 {
						...
						if err != nil {
							...
							if ((strings.HasSuffix(uri, "]")) || (strings.HasSuffix(uri, "]/"))) && (uri == requestUri) {
								...
								log.V(lvl.WARNING).Infof("Subtree at list instance level returns error %v for  URI  - %v", err, uri)
								return jsonData, true, err
					...
					if strings.HasSuffix(uri, "]") || strings.HasSuffix(uri, "]/") && (len(inParamsForGet.tbl) > 0 && len(inParamsForGet.tblKey) > 0) {
						...
						if err != nil {
							xfmrLogInfo("yangDataFill failed for list instance case(\"%v\") error: %v", uri, err)
						}
						inParamsForGet.dbDataMap = dbDataMap
						...
						if err != nil {
							xfmrLogInfo("yangListDataFill failed for list case(\"%v\") error: %v", uri, err)
						}
					}
					resultMap = inParamsForGet.resultMap
					break
				} else {
					log.V(lvl.WARNING).Infof("Unknown YANG object type for path %v", xpathKeyExtRet.xpath)
					break
				}
				if done {
					break
