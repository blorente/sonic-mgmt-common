...
func XlateFuncCall(name string, params ...interface{}) (result []reflect.Value, err error) {
	if _, ok := XlateFuncs[name]; !ok {
		log.V(lvl.DEBUG).Info("Xfmr function does not exist: ", name)
		return nil, nil
	}
	if len(params) != XlateFuncs[name].Type().NumIn() {
		log.V(lvl.WARNING).Info("Error parameters not adapted")
		return nil, nil
	}
...
func TraverseDb(dbs [db.MaxDB]*db.DB, spec KeySpec, result *map[db.DBNum]map[string]map[string]db.Value,
	...
	if err != nil {
		log.V(lvl.WARNING).Info("No conversion in dbdata-xfmr")
		return err
	}
...
func traverseDbHelper(dbs [db.MaxDB]*db.DB, spec *KeySpec, result *map[db.DBNum]map[string]map[string]db.Value,
	...
	// If we have a complete key we can directly read the database and process the results.
	// If we have no key, partial key or wildcard key then the else block will get the list
	// of table keys and iterate through them.
	if spec.Key.Len() > 0 && !spec.IsPartialKey && !wildcardKey {
		...
		if spec.Ts.Name != XFMR_NONE_STRING { //Do not traverse for NONE table
			var dbKey *db.Key
			if wildcardKey {
				dbKey = db.NewKey(spec.Key.Comp...)
			} else {
				dbKey = db.NewKey("*")
			}
			tblObj, err := dbs[spec.DbNum].GetTablePattern(&spec.Ts, *dbKey)
			if err != nil {
				log.V(lvl.WARNING).Infof("GetTablePattern returned error %v for tbl(%v) in traverseDbHelper", err, spec.Ts.Name)
				return err
			}
			dbKeys, err := tblObj.GetKeys()
			if err != nil {
				log.V(lvl.WARNING).Infof("Table.GetKeys returned error %v for tbl(%v) in traverseDbHelper", err, spec.Ts.Name)
				return err
			}
			xfmrLogDebug("Found %d keys for table %v in DB %v matching key-pattern %v keys=%v", len(dbKeys), spec.Ts.Name, spec.DbNum, *dbKey, dbKeys)
			parentDbKeyStr := ""
			...
			for _, dbKey := range dbKeys {
				...
				if err != nil {
					log.V(lvl.WARNING).Infof("Table.GetEntry returned error %v for tbl(%v), and the key %v in traverseDbHelper", err, spec.Ts.Name, dbKey)
					updateDbDataMapAndKeyCache(dbKeyStr, &data, spec, result, dbTblKeyGetCache, false)
...
func XlateUriToKeySpec(uri string, requestUri string, ygRoot *ygot.GoStruct, t *interface{}, txCache interface{},
	...
	if isSonicYang(uri) {
		...
		if tblSpecInfo, ok := xDbSpecMap[tableName]; ok && keyStr != "" && hasKeyValueXfmr(tableName) {
			...
			if err != nil {
				log.V(lvl.WARNING).Infof("Value-xfmr for table(%v) & key(%v) didn't do conversion.", tableName, keyStr)
		...
		xfmrLogDebug("filled keyspec: %v; retData: %v; reqUriXpath: %v", retdbFormat, retData, reqUriXpath)
	}
		...
		if len(tableName) > 0 {
			...
			if keyStr != "" {
				if tblSpecInfo, ok := xDbSpecMap[dbFormat.Ts.Name]; ok && tblSpecInfo.hasXfmrFn {
					...
					if err != nil {
						log.V(lvl.WARNING).Infof("Value-xfmr for table(%v) & key(%v) didn't do conversion.", dbFormat.Ts.Name, keyStr)
					}
				}
...
func XlateToDb(path string, oper int, d *db.DB, yg *ygot.GoStruct, yt *interface{}, jsonPayload []byte, txCache interface{}, skipOrdTbl *bool) (map[Operation]RedisDbMap, map[string]map[string]db.Value, map[string]map[string]db.Value, error) {
	...
	requestUri := path
	allModeljsonData := make(map[string]interface{})
	...
	if err == nil {
		err = json.Unmarshal(jsonBytes, &allModeljsonData)
	}
	if err != nil {
	...
	for modelName := range allModeljsonData {
		if strings.Contains(path, modelName) {
			jsonData[modelName] = allModeljsonData[modelName]
			break
		}
	}
	...
	case CREATE:
		...
		if err != nil {
			log.V(lvl.WARNING).Info("Data translation from YANG to db failed for create request.")
		}
	...
	case UPDATE:
		...
		if err != nil {
			log.V(lvl.WARNING).Info("Data translation from YANG to db failed for update request.")
		}
	...
	case REPLACE:
		...
		err = dbMapUpdate(d, yg, opcode, path, requestUri, jsonData, result, yangDefValMap, yangAuxValMap, txCache)
		if err != nil {
			log.V(lvl.WARNING).Info("Data translation from YANG to db failed for replace request.")
		}
	...
	case DELETE:
		...
		if err != nil {
			log.V(lvl.WARNING).Info("Data translation from YANG to db failed for delete request.")
		}
	}
...
func XlateFromDb(uri string, ygRoot *ygot.GoStruct, dbs [db.MaxDB]*db.DB, data RedisDbMap, txCache interface{}, inParamsForGet xlateFromDbParams) ([]byte, bool, error) {
	...
	if err != nil {
		log.V(lvl.DEBUG).Infof("Cannot perform GET Operation on URI %v due to - %v", uri, err)
		return []byte(""), true, err
	}
	if !exists {
	...
	if isSonicYang(uri) {
		...
		if tableName != "" {
			...
			if !ok {
				log.V(lvl.WARNING).Infof("No entry in xDbSpecMap for xpath %v", tableName)
			} else {
				cdb = dbInfo.dbIndex
			}
						...
						if (yangNodeType == YANG_LEAF_LIST) || (yangNodeType == YANG_LEAF) {
							...
							if (yangNodeType == YANG_LEAF_LIST) && ((strings.HasSuffix(uri, "]")) || (strings.HasSuffix(uri, "]/"))) {
								...
								if dbSpecFieldInfo.xfmrValue != nil {
									...
									if err != nil {
										log.V(lvl.WARNING).Infof("value-xfmr:fldpath(\"%v\") val(\"%v\"):err(\"%v\").", dbSpecField, leafListInstVal, err)
										return []byte(""), true, err
									}
								...
								if leafListInstExists(dbData[cdb][tableName][keyStr].Field[fieldName], leafListInstVal) {
									...
									*/
									log.V(lvl.INFO).Info("Queried leaf-list instance exists.")
	...
	xfmrLogDebug("Payload generated : %v", payload)

	if err != nil {
		log.V(lvl.WARNING).Info("Couldn't create json response from DB data.")
	...
	if tableName != "" && keyStr != "" && fieldName != "" {
		if data[tableName][keyStr].Field != nil {
			...
			if fldValExists {
				...
				log.V(lvl.WARNING).Infof("Field %v doesn't exist in table - %v, instance - %v", fieldName, tableName, keyStr)
				err = tlerr.NotFoundError{Format: "Resource not found"}
			}
		}
	}
...
func CallRpcMethod(path string, body []byte, dbs [db.MaxDB]*db.DB) ([]byte, error) {
	...
	if rpcFunc != "" {
		...
		if len(data) > 0 {
			if len(data) == RPC_XFMR_RET_ARGS {
				...
				if data[RPC_XFMR_RET_ERR_INDX].Interface() != nil {
					...
					if err != nil {
						log.V(lvl.WARNING).Infof("Transformer function(\"%v\") returned error - %v.", rpcFunc, err)
					}
				}
			}
		...
		log.V(lvl.WARNING).Info("Not supported RPC", path)
...
func IsTerminalNode(uri string) (bool, error) {
	...
	if xpathData, ok := xYangSpecMap[xpath]; ok {
		...
		log.V(lvl.WARNING).Infof("xYangSpecMap data not found for xpath : %v", xpath)
...
func tableKeysToBeSorted(tblNm string) bool {
	...
	*/
	...
	if dbSpecInfo, ok := xDbSpecMap[tblNm]; ok {
		...
		log.V(lvl.WARNING).Info("xDbSpecMap data not found for ", tblNm)
	}
