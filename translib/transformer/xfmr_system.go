...
func getSystemMountPoint(mountPoint MountPoint, sysMountPoint *ocbinds.OpenconfigSystem_System_MountPoints_MountPoint, name string) {
	...
	log.V(lvl.DEBUG).Infof("getSystemMountPoint: %s", mountPoint.Name)
	...
	if !ok {
		log.V(lvl.ERROR).Infof("Filesystem type unrecognized: %v", mountPoint.Type)
...
func getSystemMountPoints(mountPoints map[string]MountPoint, sysMountPoints *ocbinds.OpenconfigSystem_System_MountPoints) {
	for name, mountPoint := range mountPoints {
		log.V(lvl.DEBUG).Infof("getSystemMountPoints %s", name)
		sysMountPoint, err := sysMountPoints.NewMountPoint(name)
		if err != nil {
			log.V(lvl.ERROR).Infof("sysMountPoints.NewMountPoint failed %s", err.Error())
			return
		}
...
var DbToYang_sys_mount_points_xfmr SubTreeXfmrDbToYang = func(inParams XfmrParams) error {
	log.V(lvl.DEBUG).Info("DbToYang_sys_mount_points_xfmr: entered")
	sysObj := getAppRootObject(inParams)
	mount_points, err := getMountPointsFromDb(inParams.dbs[db.StateDB])
	if err != nil {
		log.V(lvl.ERROR).Infof("getMountPointsFromDb failed, err: %s", err.Error())
		return err
	}
	...
	if name, ok := path.Vars["name"]; ok {
		if mount_point, ok := mount_points[name]; ok {
			for _, value := range sysObj.MountPoints.MountPoint {
				ygot.BuildEmptyTree(value)
				log.V(lvl.DEBUG).Info("DbToYang_sys_mount_points_xfmr: getSystemMountPoint")
				getSystemMountPoint(mount_point, value, name)
			}
		}
	} else {
		log.V(lvl.DEBUG).Info("DbToYang_sys_mount_points_xfmr: getSystemMountPoints")
		getSystemMountPoints(mount_points, sysObj.MountPoints)
	}

	log.V(lvl.DEBUG).Info("DbToYang_sys_mount_points_xfmr: exited")
	return nil
}
...
var Subscribe_sys_mount_points_xfmr SubTreeXfmrSubscribe = func(inParams XfmrSubscInParams) (XfmrSubscOutParams, error) {
	...
	log.V(lvl.DEBUG).Infof("+++ Subscribe_sys_mount_points_xfmr (%v) +++", inParams.uri)
	if key == "" {
		if inParams.subscProc != TRANSLATE_SUBSCRIBE {
			/* no need to verify dB data if we are requesting ALL mount points */
			log.V(lvl.DEBUG).Infof("+++ Subscribe_sys_mount_points_xfmr end with virtual table")
			return XfmrSubscOutParams{isVirtualTbl: true}, nil
		}
		key = "*"
	}
	log.V(lvl.DEBUG).Infof("+++ Subscribe_sys_mount_points_xfmr end")
 ...
 * enum according to go/gpins-crash-artifact-framework */
...
var YangToDb_feature_labels_xfmr SubTreeXfmrYangToDb = func(inParams XfmrParams) (map[string]map[string]db.Value, error) {
	if inParams.oper == DELETE {
		...
		log.V(lvl.DEBUG).Infof("Deleting feature-label from ConfigDB: %v", label)
		return map[string]map[string]db.Value{
	...
	if inParams.requestUri == "/openconfig-system:system" && inParams.oper == REPLACE {
		...
		for _, key := range keys {
			...
			if err != nil {
				log.V(lvl.ERROR).Infof("Could not parse feature label key to uint32: %v", err)
				continue
			}
		...
		if len(deleteLabels) > 0 {
			log.V(lvl.DEBUG).Infof("Deleting feature-labels that are not in the config: %v", deleteLabels)
			updateSubOpDataMap(map[db.DBNum]map[string]map[string]db.Value{
				db.ConfigDB: {
	...
	log.V(lvl.DEBUG).Infof("YangToDb_feature_labels_xfmr resMap: %v", resMap)

	return resMap, nil
}

var DbToYang_feature_labels_xfmr SubTreeXfmrDbToYang = func(inParams XfmrParams) error {
	log.V(lvl.DEBUG).Infof("DbToYang_feature_labels_xfmr inParams: %#v", inParams)
	sysObj := getAppRootObject(inParams)
	cfgDb := inParams.dbs[db.ConfigDB]
	if cfgDb == nil {
		var err error
	...
	log.V(lvl.DEBUG).Infof("DbToYang_feature_labels_xfmr labels: %v", labels)
	...
	for _, label := range labels {
		...
		if !ok {
			if flObj, err = sysObj.FeatureLabels.NewFeatureLabel(fl); err != nil {
				log.V(lvl.ERROR).Infof("Could not create new feature-label object: %v", err)
				continue
			}
		}
...
var YangToDb_system_diag_xfmr SubTreeXfmrYangToDb = func(inParams XfmrParams) (map[string]map[string]db.Value, error) {
	log.V(lvl.DEBUG).Info("YangToDb_system_diag_xfmr: ", inParams.uri)
	pathInfo := NewPathInfo(inParams.uri)
	targetUriPath, _ := getYangPathFromUri(pathInfo.Path)
	...
	log.V(lvl.DEBUG).Info("YangToDb_system_diag_xfmr targetUriPath: ", targetUriPath)
	sysObj := getAppRootObject(inParams)
	diagObj := sysObj.Diag
		...
		for ppn, pp := range diagObj.PortProfiles.PortProfile {
			...
			if pp.Queues != nil && pp.Queues.Queue != nil {
				...
				for qn, q := range pp.Queues.Queue {
					...
					if err != nil {
						log.V(lvl.ERROR).Infof("YangToDb_system_diag_xfmr couldn't find qid for qname=%s: ", qn)
						return nil, err
					}
...
func populateDiagPortProfiles(diagObj *ocbinds.OpenconfigSystem_System_Diag, cfgDb, stateDb *db.DB) error {
	...
	for _, key := range keys {
		...
		if !ok {
			...
			if err != nil {
				log.V(lvl.ERROR).Infof("unable to allocate new port profile obj %v", err)
				return err
			}
		}
		...
		if err == nil {
			for qid, drt := range entry.Field {
				...
				if err != nil {
					log.V(lvl.ERROR).Infof("unable to map qid(%v) to name %v", qid, err)
					continue
				}
				qObj, err := ppObj.Queues.NewQueue(qname)
				if err != nil {
					log.V(lvl.ERROR).Infof("unable to allocate new queue obj %v", err)
					return err
				}
...
var Subscribe_system_diag_xfmr SubTreeXfmrSubscribe = func(inParams XfmrSubscInParams) (result XfmrSubscOutParams, err error) {
	log.V(lvl.DEBUG).Infof("Subscribe_system_diag_xfmr: %+v", inParams)
