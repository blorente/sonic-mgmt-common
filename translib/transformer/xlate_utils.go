...
func mapCopy(destnMap map[string]map[string]db.Value, srcMap map[string]map[string]db.Value) {
	...
	for table, tableData := range srcMap {
		...
		for rule, ruleData := range tableData {
			if dm, ok := destnMap[table][rule]; !ok || dm.Field == nil {
...
func dbKeyToYangDataConvert(uri string, requestUri string, xpath string, tableName string, d *db.DB, dbs [db.MaxDB]*db.DB,
	...
	if _, ok := xYangSpecMap[xpath]; ok {
		if xYangSpecMap[xpath].yangEntry == nil {
			log.V(lvl.WARNING).Infof("Yang Entry not available for xpath %v", xpath)
			return nil, "", "", nil
		}
	}
	...
	if len(keyNameList) > 1 && !keyNameValSame && !oneOnOnemapping {
		log.V(lvl.WARNING).Infof("No key transformer found for multi element yang key mapping to a single redis key string, for uri %v", uri)
		errStr := fmt.Sprintf("Error processing key for list %v", uri)
		err = fmt.Errorf("%v", errStr)
		return rmap, uriWithKey, relUri, err
	}

	/* In the case of nested lists (e.g. /a/b[]/c/d[]) our DB key and therefore
	 * keyDataList would have all values while our keyNameList only has the names
	 * of the trailing key field(s) (e.g. d's key).  Handle this by ignoring the
	 * first few entries in the keyDataList since they do not correspond to the
	 * field values we are looking for here. */
	if !oneOnOnemapping && len(keyDataList) > len(keyNameList) {
		extra := len(keyDataList) - len(keyNameList)
		xfmrLogDebug("dbKeyToYangDataConvert: nested lists, trimming data list (keyNameList: %v keyDataList: %v trimmed %v)", keyNameList, keyDataList, keyDataList[extra:])
		keyDataList = keyDataList[extra:]
	}
...
func getYangPathFromUri(uri string) (string, error) {
	...
	if err != nil {
		log.V(lvl.WARNING).Infof("Error in URI to path conversion: %v", err)
		return "", err
	}

	yangPath, yperr := ygot.PathToSchemaPath(path)
	if yperr != nil {
		log.V(lvl.WARNING).Infof("Error in Gnmi path to Yang path conversion: %v", yperr)
		return "", yperr
	}
...
func sonicKeyDataAdd(dbIndex db.DBNum, keyNameList []string, xpathPrefix string, listNm string, keyStr string, resultMap map[string]interface{}, oper Operation, isPartialKeyForOc bool) {
	...
	for i, keyName := range keyNameList {
		...
		if dbEntry == nil {
			log.V(lvl.WARNING).Infof("xDbSpecMap entry not found or is nil for xpath %v, hence data-type conversion cannot happen", keyXpath)
		} else if resVal != "" {
			yngTerminalNdDtType := dbEntry.Type.Kind
			var err error
			resVal, _, err = DbToYangType(yngTerminalNdDtType, keyXpath, keyValList[i], oper)
			if err != nil {
				log.V(lvl.WARNING).Infof("Failed to convert data-type for xpath %v. Key-xfmr recommended if data types differ", keyXpath)
				resVal = keyValList[i]
			}
		}
		...
		if yangEntry != nil {
			for _, k := range strings.Split(yangEntry.Key, " ") {
				...
				if _, keyXpathEntryOk := xYangSpecMap[keyXpath]; !keyXpathEntryOk {
					log.V(lvl.WARNING).Infof("No entry found in xYangSpec map for xapth %v", keyXpath)
					err = fmt.Errorf("No entry found in xYangSpec map for xapth %v", keyXpath)
					break
				}
				keyYangEntry := yangEntry.Dir[k]
				if keyYangEntry == nil {
					log.V(lvl.WARNING).Infof("Yang Entry not available for xpath %v", keyXpath)
				...
				if keyValErr != nil {
					log.V(lvl.WARNING).Infof("unmarshalJsonToDbData() didn't unmarshal for key %v with xpath %v", k, keyXpath)
					err = keyValErr
					break
				}
...
func uriModuleNameGet(uri string) (string, error) {
	...
	result := ""
	if len(uri) == 0 {
		log.V(lvl.WARNING).Info("Empty URI string supplied")
		err = fmt.Errorf("Empty URI string supplied")
		return result, err
	}
	urislice := strings.Split(uri, ":")
	if len(urislice) == 1 {
		log.V(lvl.WARNING).Infof("uri string %s does not have module name", uri)
	...
	if len(strings.Trim(result, " ")) == 0 {
		log.V(lvl.WARNING).Info("Empty module name")
...
func xpathKeyExtract(d *db.DB, ygRoot *ygot.GoStruct, oper Operation, path string, requestUri string, dbDataMap *map[db.DBNum]map[string]map[string]db.Value, subOpDataMap map[Operation]*RedisDbMap, txCache interface{}, xfmrTblKeyCache map[string]tblKeyCache, dbs [db.MaxDB]*db.DB) (xpathTblKeyExtractRet, error) {
	...
	if !ok {
		log.V(lvl.WARNING).Infof("No entry found in xYangSpecMap for xpath %v.", retData.xpath)
		return retData, err
	}
	...
	if d != nil {
		...
		log.V(lvl.WARNING).Infof("xpathKeyExtract: nil DB pointer is being passed to key transformer for that path: %v", path)
	}
			...
			*/
			...
			if xpathInfo.dbIndex != db.MaxDB {
				...
				if dbs[cdb] != nil {
					...
					log.V(lvl.WARNING).Infof("xpathKeyExtract: dbs array: %v does not have the db pointer for the db index: %v and the yang path: %v", dbs, xpathInfo.dbIndex, path)
				}
			} else {
				log.V(lvl.WARNING).Infof("xpathKeyExtract: invalid xpathInfo.dbIndex: %v for the yang path: %v ", xpathInfo.dbIndex, path)
			}
...
func xpathKeyExtractForGet(d *db.DB, ygRoot *ygot.GoStruct, oper Operation, path string, requestUri string,
	...
	xpathInfo, ok := xYangSpecMap[retData.xpath]
	if !ok {
		log.V(lvl.WARNING).Infof("xpathKeyExtractForGet: No entry found in xYangSpecMap for xpath %v; uri: %v", retData.xpath, path)
		return retData, err
	}
	...
	if d != nil {
		...
		log.V(lvl.WARNING).Infof("xpathKeyExtractForGet: nil DB pointer is being passed to key transformer for that path: %v; dbIndex: %v", path, cdb)
	}

	pathIdx := len(pathList)
	childListNodePathIdx := 0

	xfmrLogDebug("path elements are : %v; dbIndex: %v; requestUriYangType: %v; pathIdx: %v", pathList, cdb, requestUriYangType, pathIdx)
	...
	if tblKeyOk {
		...
		xfmrLogDebug("found key xfmr: %v in cache;  retData.xpath: %v; pathIdx: %v;"+
		...
		for len(ygXpathInfo.xfmrKey) == 0 {
			xfmrLogDebug("ygXpathInfo.fieldName %s (ygXpath %s)", ygXpathInfo.fieldName, ygXPath)
			/* Google: When we have a subtree transformer we still want to execute this block.
			 * by not executing it we are left with an empty key (keyStr == "") which causes
			 * the framework to read all keys even when a specific key is specified. */
				...
				if ygXpathInfo.yangEntry.IsList() {
					...
					if ygXpathInfo.dbIndex != db.MaxDB {
						...
						if dbs[cdb] != nil {
							...
							log.V(lvl.WARNING).Infof("dbs array: %v does not have the db pointer for the db index: %v "+
					...
					if ygXpathInfo.keyName != nil {
						// Fixed key name (e.g. sonic-extension "key-name" annotation)
						...
						*/
						...
						if tblPtr != nil && *tblPtr != XFMR_NONE_STRING {
							...
							if tErr != nil {
								xfmrLogDebug("error: %v; in table transformer: %v; curPathWithKey: %v; "+
						...
						xfmrLogDebug("No key-xfmr at list %v, 1:1 mapping case. Concatenating YANG keys;"+
						...
						if pathInfo != nil /*&& (len(pathInfo.Vars) > 0)*/ {
							...
							for _, keyNm := range keyNmList {
								...
								} else if xpathInfo.tableName != nil && *xpathInfo.tableName != XFMR_NONE_STRING {
									// The key was not specified, assume a wildcard.  This may happen when requesting
									// a path of /a/b/c instead of /a/b/c[key=*]
									pathKeyStr += "*"
								}
							}
						...
						if hasSameOcSonicKeys(curPathWithKey, ygXPath, tableName) {
							...
							xfmrLogInfo("path traversal; sonic and oc yang has same keys; no need to traverse backward; "+
					...
					xfmrLogDebug("path traversal; curr path: %v; pathIdx: %v; "+
					...
					xfmrLogDebug("pathIdx %v, keyStr=\"%s\" + \"%s\"", pathIdx, *ygXpathInfo.keyName, keyStr)
					keyStr = *ygXpathInfo.keyName + keyStr
				}
			}
			...
			if tIdx > 0 {
				...
				xfmrLogInfo("path traversal; reach first index of the path; last path index: %v; "+
			...
			if ygXpathInfoTmp, ok := xYangSpecMap[ygXPath]; !ok || ygXpathInfoTmp == nil {
				log.V(lvl.WARNING).Infof("xYangSpecMap does not have the yangXpathInfo for the yang "+
			...
			xfmrLogDebug("path traversal; pathIdx: %v; curPathWithKey: %v;"+
				" keyStr: %v; ygXPath: %v", pathIdx, curPathWithKey, keyStr, ygXPath)
		}

		xfmrLogDebug("curPathWithKey: %v; ygXpathInfo.xfmrKey: %v; pathIdx: %v; "+
			"childListNodePathIdx: %v; xpath: %v, keyStr: %v", curPathWithKey, ygXpathInfo.xfmrKey, pathIdx, childListNodePathIdx, retData.xpath, keyStr)
	...
	if len(ygXpathInfo.xfmrKey) > 0 {
		...
		if xfmrTblKeyCache != nil {
			if tkCache, _ok := xfmrTblKeyCache[curPathWithKey]; _ok {
				if len(tkCache.dbKey) != 0 {
					keyStr = tkCache.dbKey
					xfmrLogDebug("db key %v found in xfmrTblKeyCache for the path: %v; key xfmr: %v", keyStr, curPathWithKey, ygXpathInfo.xfmrKey)
				}
			}
		}
		...
		if len(keyStr) == 0 {
			...
			if ygXpathInfo.dbIndex != db.MaxDB {
				...
				if dbs[cdb] != nil {
					...
					log.V(lvl.WARNING).Infof("xpathKeyExtractForGet: dbs array: %v does not have the db pointer for the db index: %v; curPathWithKey: %v", dbs, ygXpathInfo.dbIndex, curPathWithKey)
				}
			} else {
				log.V(lvl.WARNING).Infof("xpathKeyExtractForGet: invalid ygXpathInfo.dbIndex: %v for the curPathWithKey: %v ", ygXpathInfo.dbIndex, curPathWithKey)
			}
			...
			if err != nil {
				retData.dbKey, retData.tableName, retData.xpath = "", "", ""
				log.V(lvl.WARNING).Infof("xpathKeyExtractForGet: keyXfmr %v failed at current path: %v; error: %v. Please check the key-xfmr;"+
	...
	if tblPtr != nil && *tblPtr != XFMR_NONE_STRING {
		...
		if tErr != nil {
			log.V(lvl.WARNING).Infof("xpathKeyExtractForGet: got error: %v from tblNameFromTblXfmrGet; reqPath: %v;  db index: %v; "+
	...
	xfmrLogDebug("retData: %#v; err: %v", retData, err)
	return retData, err
}
...
func dbTableFromUriGet(d *db.DB, ygRoot *ygot.GoStruct, oper Operation, uri string, requestUri string, subOpDataMap map[Operation]*RedisDbMap, txCache interface{}, xfmrTblKeyCache map[string]tblKeyCache) (string, error) {
	...
	if !ok {
		log.V(lvl.WARNING).Infof("No entry found in xYangSpecMap for xpath %v.", xPath)
...
func xfmrLogInfo(format string, args ...interface{}) {
	...
	log.V(lvl.DEBUG).Infof(fNmLnoStr+format, args...)
}
				...
				if len(keyMap) == len(specListInfo.keyList) {
					for _, kname := range specListInfo.keyList {
						...
						if kInfo, ok := xDbSpecMap[keyXpath]; ok && xDbSpecMap[keyXpath].xfmrValue != nil && curKeyVal != "" {
							...
							if err != nil {
								log.V(lvl.WARNING).Infof("value-xfmr: keypath(\"%v\") value (\"%v\"):err(%v).",
						...
						if _, ok := skipKeySet[dbKey]; !ok {
							for fld, val := range fldData.Field {
								...
								if ok && fInfo.xfmrValue != nil {
									...
									if err != nil {
										log.V(lvl.WARNING).Infof("value-xfmr:fldpath(\"%v\") val(\"%v\"):err(\"%v\").",
											fldXpath, val, err)
										return err
									}
...
func (ygtXltr *ygotXlator) validate() error {
	...
	if len(uri) == 0 {
		...
		log.V(lvl.WARNING).Info("ygotXlator: validate: " + ygtXltr.ygotCtx.err.Error())
...
func (ygtXltr ygotXlator) uriToPath() (*gnmi.Path, error) {
	...
	if err != nil {
		...
		log.V(lvl.WARNING).Info(ygtXltr.ygotCtx.err)
...
func dbTableExists(d *db.DB, tableName string, dbKey string, oper Operation) (bool, error, db.Value) {
	...
	if len(tableName) > 0 {
		...
		if strings.Contains(dbKey, "*") {
			keys, derr := d.GetKeysPattern(dbTblSpec, db.Key{Comp: []string{dbKey}})
			if derr != nil {
				log.V(lvl.WARNING).Infof("Failed to get keys for tbl(%v) dbKey pattern %v error: %v", tableName, dbKey, derr)
			...
			if len(keys) > 0 {
				...
				log.V(lvl.WARNING).Infof("dbKey %v does not exist in DB for table %v", dbKey, tableName)
				err = tlerr.NotFound("Resource not found")
				return false, err, db.Value{}
			}
		} else {
			...
			if derr != nil {
				log.V(lvl.WARNING).Infof("GetEntry failed for table: %v, key: %v err: %v", tableName, dbKey, derr)
				err = tlerr.NotFound("Resource not found")
				return false, err, db.Value{}
			}
		...
		log.V(lvl.WARNING).Info("Empty table name received")
...
func contentQParamYangNodeProcess(uri string, yangNdInfo contentQPSpecMapInfo, qParams QueryParams) (bool, error) {
	...
	*/
	...
	content_mismatch_err := QUERY_CONTENT_MISMATCH_ERR
