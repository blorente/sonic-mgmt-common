...
func xfmrHandlerFunc(inParams XfmrParams, xfmrFuncNm string, ygotCtx *ygotUnMarshalCtx) error {
	...
	if (ret != nil) && (len(ret) > 0) {
		...
		if ret[DBTY_SBT_XFMR_RET_ERR_INDX].Interface() != nil {
			...
			if err != nil {
				log.V(lvl.DEBUG).Infof("xfmr function(\"%v\") returned error - %v.", xfmrFuncNm, err)
			}
		}
	}
	if (err == nil) && inParams.queryParams.isEnabled() && !(*inParams.pruneDone) {
		log.V(lvl.DEBUG).Infof("xfmrPruneQP: func %v URI %v, requestUri %v",
	...
	if (ret != nil) && (len(ret) > 0) {
		if len(ret) == DBTY_FLD_XFMR_RET_ARGS {
			...
			if ret[DBTY_FLD_XFMR_RET_ERR_INDX].Interface() != nil {
				...
				if err != nil {
					log.V(lvl.DEBUG).Infof("xfmr function(\"%v\") returned error - %v.", xfmrFieldFuncNm, err)
				}
			}
		}
	...
	if (ret != nil) && (len(ret) > 0) {
		if len(ret) == TBL_XFMR_RET_ARGS {
			...
			if ret[TBL_XFMR_RET_ERR_INDX].Interface() != nil {
				...
				if err != nil {
					log.V(lvl.DEBUG).Infof("xfmr function(\"%v\") returned error - %v.", xfmrTblFunc, err)
					return retTblLst, err
				}
			}
		}
	...
	if (ret != nil) && (len(ret) > 0) {
		if len(ret) == YTDB_FLD_XFMR_RET_ARGS {
			...
			if ret[YTDB_FLD_XFMR_RET_ERR_INDX].Interface() != nil {
				...
				if err != nil {
					log.V(lvl.DEBUG).Infof("Transformer function(\"%v\") returned error - %v.", xfmrValueFuncNm, err)
					return "", err
				}
			}
		}
	...
	if (ret != nil) && (len(ret) > 0) {
		if len(ret) == POST_XFMR_RET_ARGS {
			...
			if ret[POST_XFMR_RET_ERR_INDX].Interface() != nil {
				...
				if err != nil {
					log.V(lvl.DEBUG).Infof("Transformer function(\"%v\") returned error - %v.", xfmrPost, err)
					return retData, err
				}
			}
		}
...
func preXfmrHandlerFunc(xfmrPre string, inParams XfmrParams) error {
	...
	if err != nil {
		log.V(lvl.WARNING).Infof("Pre-transformer function(\"%v\") returned error - %v.", xfmrPre, err)
		return err
	}
	if (ret != nil) && (len(ret) > 0) {
		if ret[PRE_XFMR_RET_ERR_INDX].Interface() != nil {
			...
			if err != nil {
				log.V(lvl.DEBUG).Infof("Transformer function(\"%v\") returned error - %v.", xfmrPre, err)
				return err
			}
		}
	}
	...
	if (ret != nil) && (len(ret) > 0) {
		if len(ret) == DBTY_KEY_XFMR_RET_ARGS {
			...
			if ret[DBTY_KEY_XFMR_RET_ERR_INDX].Interface() != nil {
				err = ret[DBTY_KEY_XFMR_RET_ERR_INDX].Interface().(error)
				if err != nil {
					log.V(lvl.DEBUG).Infof("Transformer function(\"%v\") returned error - %v.", xfmrKeyNm, err)
					return retVal, err
				}
			}
		}
	...
	if (ret != nil) && (len(ret) > 0) {
		if len(ret) == SUBSC_SBT_XFMR_RET_ARGS {
			...
			if ret[SUBSC_SBT_XFMR_RET_ERR_INDX].Interface() != nil {
				...
				if err != nil {
					log.V(lvl.DEBUG).Infof("Subscribe xfmr function(\"%v\") returned error - %v.", xfmrFuncNm, err)
					return retVal, err
				}
			}
		}
