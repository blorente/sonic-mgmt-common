...
func main() {
	...
	flag.Parse()
	log.V(lvl.INFO).Info("operation =", *operationPtr)
	log.V(lvl.INFO).Info("uri =", *uriPtr)
	log.V(lvl.INFO).Info("payload =", *payloadFilePtr)
	...
	if *operationPtr == "create" {
		...
		log.V(lvl.INFO).Info("Response payload =", string(resp.Payload))
	} else if *operationPtr == "getmodels" {
		models, _ := translib.GetModels()
		log.V(lvl.INFO).Info("Models =", models)
	} else if *operationPtr == "supportsubscribe" {
		paths := strings.Split(*uriPtr, ",")
		log.V(lvl.INFO).Info("Paths =", paths)

		resp, _ := translib.IsSubscribeSupported(paths)

		for i, path := range paths {
			log.V(lvl.INFO).Info("Response returned for path=", path)
			log.V(lvl.INFO).Info(*(resp[i]))
		}
		...
		log.V(lvl.INFO).Info("Subscribe completed")
		for {
			log.V(lvl.INFO).Info("Before calling Get")
			items, err := q.Get(1)
			log.V(lvl.INFO).Info("After calling Get")
			...
			log.V(lvl.INFO).Info("SubscribeResponse received =", string(resp.Payload))
			log.V(lvl.INFO).Info("IsSync complete = ", resp.SyncComplete)
		...
		log.V(lvl.INFO).Info("Invalid Operation")
	}
}
