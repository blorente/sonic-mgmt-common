...
func (sess *Session) StartTx(d *db.DB, w []db.WatchKeys,
	...
	glog.V(lvl.INFO).Infof("cs.StartTx:[%s]: Begin", sess.token)
	var e error
...
func (sess *Session) CommitTx(d *db.DB) error {
	glog.V(lvl.INFO).Infof("cs.CommitTx:[%s]: Begin", sess.token)
	var e error
...
func (sess *Session) AbortTx(d *db.DB) error {
	glog.V(lvl.INFO).Infof("cs.AbortTx:[%s]: Begin", sess.token)
	var e error
	if (uCS != nil) && (d == uCS.ccDB) {
		e = d.Rollback2SP()
	} else {
