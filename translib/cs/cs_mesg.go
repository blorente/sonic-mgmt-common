...
func (sess *Session) SendMesg(mesg string, userName string) error {
	glog.V(lvl.INFO).Infof("SendMesg:[%s]:Begin: %s: from %s", sess.token, mesg, userName)
	...
	if sess.IsConfigSession() && sess.IsPidActive() {
		...
		if ttyName, err = os.Readlink(link); err != nil {
			...
			if n, err = tty.WriteString(fMesg); err != nil {
				glog.V(lvl.INFO).Infof("SendMesg: WriteString(%s) n %d err %v", fMesg, n,
					err)
	...
	glog.V(lvl.INFO).Infof("SendMesg:[%s]:End:", sess.token)
	return err
}
