...
func (cs *configSession) IsPidActive() bool {
	...
	if pid == 0 {
		...
		glog.V(lvl.INFO).Infof("cs.IsPidActive[%s]: Inactive pid %v", cs.token, pid)
		return false
	}
...
func newCS(name string, username string, roles []string, pid int32) (*configSession, error) {
	glog.V(lvl.INFO).Infof("newCS: %s %s %v %d", name, username, roles, pid)
	...
	glog.V(lvl.INFO).Infof("newCS[%s]: %s %s %v %d", token, name, username, roles, pid)
	return uCS, nil
}

func resumeCS(name string, roles []string, pid int32) (*configSession, error) {
	glog.V(lvl.INFO).Infof("resumeCS: %s %s %v", name, roles, pid)
	...
	glog.V(lvl.INFO).Infof("resumeCS[%s]: %s %s %v", uCS.token, name, roles, pid)
	return uCS, nil
}

func suspendCS(name string) (*configSession, error) {
	glog.V(lvl.INFO).Infof("suspendCS: %s", name)
	...
	glog.V(lvl.INFO).Infof("suspendCS[%s]: %s", uCS.token, name)
	return uCS, nil
}
...
func commitCS(name string, label string, isConfirmNeeded bool) (error, error, error) {
	glog.V(lvl.INFO).Infof("commitCS: name: %s, label: %s", name, label)
	...
	if uCS == nil || uCS.state != cs_STATE_ACTIVE {
		...
		if isConfirmNeeded {
			...
			if errSc = db.ConfigDBUnlock(uCS.token); errSc != nil {
				glog.V(lvl.WARNING).Infof("commitCS: db.ConfigDBUnlock errSc %+v", errSc)
			}
			uCS = nil
		}
	}
	...
	glog.V(lvl.INFO).Infof("commitCS[%s]: end", token)
...
func deleteCS(name string) (error, error) {
	glog.V(lvl.INFO).Infof("deleteCS: %s", name)
	...
	glog.V(lvl.INFO).Infof("deleteCS[%s]: %s err %s errU %s", uCS.token, name, err, errU)
...
func createCpHistory(label string) error {
	...
	if errSh = CreateCpHistEntry(label, uCS.token, uCS.username,
		...
		glog.V(lvl.WARNING).Infof("commitCS: CreateCpHistEntry errSh %+v", errSh)
	}
	return errSh
}
