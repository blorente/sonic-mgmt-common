...
func saveRunningConfig(username string) (string, error) {
	...
	if queryResult.Err != nil {
		glog.V(lvl.INFO).Infof("Error: %s", queryResult.Err.Error())
...
func processCommitConfirm(sess *Session, label string) (bool, CsStatus) {
	...
	if sess.configSession.commitState != cs_STATE_CONFIRM_TIMER {
		glog.V(lvl.INFO).Infof("Commit:[%s] Invalid confirm before commit.", sess.token)
	...
	if sess.configSession.commitTimer != nil {
		glog.V(lvl.INFO).Infof("Confirm :[%s]:Stop commit timer.", sess.token)
...
func processCommitTimer(sess *Session, timeout int, rollbackCfg string) (bool, CsStatus) {
	...
	go func() {
		...
		glog.V(lvl.INFO).Infof("Commit:[%s]: Timer goroutine. ", sess.token)

		select {
		case <-commitCh:
			glog.V(lvl.INFO).Infof("Commit:[%s]: goroutine completed. ", sess.token)
			return
		...
		case timeouttime := <-commitTimer.C:
			...
			glog.V(lvl.INFO).Infof("Commit:[%s]:Timer timeout %s, Reload initiated.", sess.token, timeouttime)
...
func (sess *Session) Commit(label string,
	...
	glog.V(lvl.INFO).Infof("Commit:[%s]:Begin: label: %s, timeout: %d", sess.token, label, timeout)
	...
	if sess.IsConfigSession() {
		...
		case cs_STATE_ACTIVE:
			...
			if sess.configSession.commitState == cs_STATE_CONFIRM_TIMER {
				glog.V(lvl.INFO).Infof("Commit:[%s] Commit timer running.", sess.token)
		...
		default:
			...
			glog.V(lvl.INFO).Infof("Commit: Session In Use. State %v", sess.configSession.state)
			success = false
	...
	glog.V(lvl.INFO).Infof("Commit:[%s]:End:", sess.token)
	return success, status
}
