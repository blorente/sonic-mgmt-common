...
func (sess *Session) StartOrResume(pid int32) (string, bool, CsStatus) {
	glog.V(lvl.INFO).Infof("StartOrResume:[%s]:Begin: %s %d", sess.name, sess.token,
		sess.pid)
	...
	if sess.IsConfigSession() {
		...
		case cs_STATE_ACTIVE:
			...
			glog.V(lvl.INFO).Infof("StartOrResume: Session In Use. Check if PID is stale")
			if sess.IsPidActive() {
				success = false
				status = CsStatusNotAllowed{}
				break
			}
			glog.V(lvl.INFO).Infof("StartOrResume: PID is stale. Suspend Session. Retrying")
			suspendCS(sess.name)
			fallthrough
		...
		case cs_STATE_SUSPENDED:
			...
			if cs, err := resumeCS(sess.name, sess.roles,
				...
				glog.V(lvl.INFO).Infof("StartOrResume: Resumed Session")
		...
		if cs, err := newCS(sess.name, sess.username, sess.roles,
			...
			glog.V(lvl.INFO).Infof("StartOrResume: New Session")
			sess.configSession = *cs
			token = cs.token
			success = true
	...
	glog.V(lvl.INFO).Infof("StartOrResume:[%s]:End:", sess.token)
