...
func GetSession(name string, token string, username string, roles []string,
	...
	glog.V(lvl.INFO).Infof("GetSession:[%s]:[%s]: %s %v %d %#v", name, token,
	...
	for _, opt := range opts {
		...
		case GSOname:
			gsoName = true
		default:
			glog.V(lvl.WARNING).Infof("GetSession: Invalid Option. %v", opt)
		}
	}
	...
	if len(token) == 0 && !gsoName {
		...
		if gsoName && ucs.name != name {
			glog.V(lvl.WARNING).Infof("GetSession: name mismatch %s != %s", name, ucs.name)
			err = CsStatusInvalidSession{Tag: ErrTagNameNotFound}

		} else if (!gsoName) && (ucs.token != token) {
			glog.V(lvl.WARNING).Infof("GetSession: token mismatch %s != %s", token, ucs.token)
			err = CsStatusInvalidSession{Tag: ErrTagTokenNotFound}

		} else if gsoStrict && (ucs.username != username) {
			glog.V(lvl.WARNING).Infof("GetSession: user mismatch %s != %s", username,
				ucs.username)
			err = CsStatusInvalidSession{Tag: ErrTagInvalidUser}

		} else if gsoStrict && ucs.IsPidActive() && (ucs.pid != pid) {
			glog.V(lvl.WARNING).Infof("GetSession: pid mismatch %d != %d", pid, ucs.pid)
			...
			glog.V(lvl.INFO).Infof("GetSession:[%s]:[%s]: Found Session %s", name, token,
				session.token)
		}

	} else if len(token) != 0 {
		glog.V(lvl.WARNING).Infof("GetSession[%s][%s]: Token. No Session", name, token)
		err = CsStatusInvalidSession{Tag: ErrTagTokenNotFound}
	}
