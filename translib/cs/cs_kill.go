...
func (sess *Session) Kill(killActive bool, user string) CsStatus {
	glog.V(lvl.INFO).Infof("Kill:[%s]:Begin: killActive=%t", sess.token, killActive)
	...
	if sess.IsConfigSession() {
		...
		case cs_STATE_ACTIVE:
			...
			if !killActive && sess.IsPidActive() {
				glog.V(lvl.INFO).Infof("Kill[%s]: Session is active on terminal %d", sess.token, sess.pid)
				status = CsStatusInvalidSession{Tag: ErrTagActive}
				break
			}

			notify = true
			glog.V(lvl.INFO).Infof("Kill:[%s]: KillActive Option", sess.token)
			fallthrough
	...
	glog.V(lvl.INFO).Infof("Kill:[%s]:End:", sess.token)
	return status
}
