...
func (nb *notificationInfoBuilder) Build() (translateSubResponse, error) {
	...
	if err != nil {
		log.V(lvl.WARNING).Infof("Error parsing path \"%s\"; err=%v", nb.pathInfo.Path, err)
	...
	if err := ymap.collect(nb, recurse); err != nil {
		log.V(lvl.WARNING).Infof("translateSubscribe failed for path: \"%s\"; err=%s", nb.pathInfo.Path, err)
