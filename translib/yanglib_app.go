...
func (app *yanglibApp) processGet(dbs [db.MaxDB]*db.DB, fmtType TranslibFmtType) (GetResponse, error) {
	glog.V(lvl.INFO).Infof("path = %s", app.pathInfo.Template)
	glog.V(lvl.INFO).Infof("vars = %s", app.pathInfo.Vars)
...
func (app *yanglibApp) copyOneModuleInfo(fromMods *ocbinds.IETFYangLibrary_ModulesState) error {
	...
	glog.V(lvl.INFO).Infof("Copying module %s@%s", key.Name, key.Revision)
...
func getYanglibInfo() (ylib *ocbinds.IETFYangLibrary_ModulesState, err error) {
	...
	if theYanglibCache == nil {
		glog.V(lvl.INFO).Infof("Building yanglib cache")
		theYanglibCache, err = newYanglibInfo()
		glog.V(lvl.INFO).Infof("Yanglib cache ready; err=%v", err)
	}
...
func (yb *yanglibBuilder) prepare() error {
	...
	glog.V(lvl.INFO).Infof("yanglibBuilder.prepare: yangDir = %s", yb.yangDir)
	glog.V(lvl.INFO).Infof("yanglibBuilder.prepare: baseURL = %s", theSchemaRootURL)
...
func (yb *yanglibBuilder) loadYangs() error {
	glog.V(lvl.INFO).Infof("Loading yangs from %s directory", yb.yangDir)
	...
	glog.V(lvl.INFO).Infof("%d yang files loaded in %s; %d ignored", parsed, time.Since(start), ignored)
	yb.yangModules = mods
	return nil
}
...
func (yb *yanglibBuilder) getSchemaURL(m *yang.Module) *string {
	...
	if len(toks) != 1 && len(toks) != 3 {
		glog.V(lvl.WARNING).Infof("Could not resolve file path for module %s; location=%s",
