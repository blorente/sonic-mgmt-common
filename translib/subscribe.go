...
type SubscribeResponse struct {
	...
	YGSMeta          ygot.YGStructMetaMap   // YGSMeta is the metadata map for Update.
	Delete           []string               // deleted paths - relative to Path
	Timestamp        int64
	SyncComplete     bool
	IsTerminated     bool
	IsOnChangeUpdate bool
}
...
func Subscribe(req SubscribeRequest) error {
	...
	paths := req.Paths
	log.V(lvl.DEBUG).Infof("[%v] Subscribe: paths = %v", sid, paths)
...
func Stream(req SubscribeRequest) error {
	sid := subscribeContextId(req.Session)
	log.V(lvl.DEBUG).Infof("[%v] Stream: paths = %v", sid, req.Paths)
...
func IsSubscribeSupported(req IsSubscribeRequest) ([]*IsSubscribeResponse, error) {
	...
	log.V(lvl.DEBUG).Infof("[%v] IsSubscribeSupported: paths = %v", reqID, paths)
	...
	log.V(lvl.DEBUG).Infof("[%v] IsSubscribeSupported: returning %d IsSubscribeResponse; err=%v", reqID, len(resp), err)
	if log.V(5) {
		for i, r := range resp {
...
func (sc *subscribeContext) startSubscribe() error {
	...
	for dbno, nGroups := range sc.dbNInfos {
		...
		if err != nil {
			log.V(lvl.WARNING).Infof("[%v] db subscribe failed -- %v", sInfo.id, err)
			cleanup(sInfo.stop)
			return err
		}
	}

	// Only allow the initial update to read the OnChange cache.
	for _, sdb := range sInfo.dbs {
		if sdb != nil {
			sdb.Opts.ReadCacheOnly = true
			defer func() { sdb.Opts.ReadCacheOnly = false }()
		}
	}
	for _, nInfo := range sc.tgtInfos {
		err := sendInitialUpdate(sInfo, nInfo)
		if err != nil {
			log.V(lvl.WARNING).Infof("[%v] init sync failed -- %v", sInfo.id, err)
			cleanup(sInfo.stop)
			return err
		}
	}
...
func (sc *subscribeContext) translateSubscribe(reqPath string, mode NotificationType) (*translatedPathInfo, error) {
	...
	log.V(lvl.DEBUG).Infof("[%v] Calling translateSubscribe for path=\"%s\", mode=%v, recurse=%v",
	...
	if err != nil {
		log.V(lvl.WARNING).Infof("[%v] translateSubscribe failed for \"%s\"; err=%v", sid, reqPath, err)
		return nil, err
	}

	if len(resp.ntfAppInfoTrgt) == 0 {
		log.V(lvl.WARNING).Infof("[%v] translateSubscribe returned nil/empty response for path: %s", sid, reqPath)
		var pType string
...
func (sc *subscribeContext) validateAppInfo(nAppInfo *notificationAppInfo, mode NotificationType) error {
	if nAppInfo == nil {
		log.V(lvl.WARNING).Infof("[%v] app returned nil notificationAppInfo", sc.id)
		return tlerr.New("internal error")
	}
	if nAppInfo.path == nil {
		log.V(lvl.WARNING).Infof("[%v] app returned nil path", sc.id)
		return tlerr.New("internal error")
	}
	if nAppInfo.isNonDB() {
...
func (sc *subscribeContext) validateNonDbPath(nAppInfo *notificationAppInfo, mode NotificationType) error {
	...
	if path.HasWildcardKey(nAppInfo.path) {
		...
		log.V(lvl.INFO).Infof("[%v] Wildcard keys not supported for non-DB path: %s", sc.id, p)
...
func (sc *subscribeContext) validateDynamicSrcPath(nAppInfo *notificationAppInfo) error {
	if e0 := path.GetElemAt(nAppInfo.path, 0); !strings.HasPrefix(e0, "openconfig-") {
		p := path.String(nAppInfo.path)
		log.V(lvl.WARNING).Infof("[%v] Dynamic table handling supported only for OC yangs; found %s", sc.id, p)
	...
	if n := path.Len(nAppInfo.path); path.HasWildcardKey(path.SubPath(nAppInfo.path, 0, n-1)) {
		p := path.String(nAppInfo.path)
		log.V(lvl.INFO).Infof("[%v] Wildcard supported only at the last element for dynamic table path; found: %s", sc.id, p)
		return tlerr.NotSupported("Wildcard keys not supported: %s", p)
	}
	return nil
}
...
func (sc *subscribeContext) validateKeyGroup(nAppInfo *notificationAppInfo) error {
	if nAppInfo.isNonDB() {
		log.V(lvl.WARNING).Infof("[%v] keyGroups  not supported for non-DB path %s", sc.id, path.String(nAppInfo.path))
		return tlerr.New("internal error")
	}
	for _, k := range nAppInfo.keyGroupComps {
		if k < 0 || k >= nAppInfo.key.Len() {
			log.V(lvl.WARNING).Infof("[%v] keyGroups %v contains invalid values (>%d), path=%s",
...
func (sc *subscribeContext) getFromSession(path string, mode NotificationType) *translatedSubData {
	...
	if sc.session != nil {
		...
		log.V(lvl.DEBUG).Infof("[%v] found trData %p from session for \"%s\"", sc.id, trData, path)
	}
	return trData
}
...
func (tpInfo *translatedPathInfo) saveToSession() {
	...
	log.V(lvl.DEBUG).Infof("[%v] set trData %p in session for \"%s\"", tpInfo.sContext.id, trData, path)
...
func (tpInfo *translatedPathInfo) segregateSampleSubpaths() []*translatedPathInfo {
	...
	log.V(lvl.DEBUG).Infof("[%v] segregate on_change and sample segments for %s", sid, tpInfo.path)
	...
	log.V(lvl.DEBUG).Infof("[%v] found %d on_change and %d sample targetInfos",
		sid, len(onchgInfos), len(sampleInfos))

	// Path does not support ON_CHANGE. No extra re-grouping required.
	if len(onchgInfos) == 0 {
		log.V(lvl.DEBUG).Infof("[%v] on_change={}; sample={%s}", sid, tpInfo.path)
		return nil
	}
	...
	log.V(lvl.DEBUG).Infof("[%v] on_change=[%s]; sample=%v", sid, tpInfo.path, samplePaths)
	return samplePInfos
}
...
func cleanup(stop chan struct{}) {
	if sInfo, ok := stopMap[stop]; ok {
		log.V(lvl.DEBUG).Infof("[%v] stopping..", sInfo.id)
