...
func TestMain(m *testing.M) {
	...
	for name, f := range cleanupFuncs {
		if err := f(); err != nil {
			...
			glog.V(lvl.INFO).Infof("Cleanup %s before tests", name)
		}
	}
	...
	for name, f := range cleanupFuncs {
		if err := f(); err != nil {
			glog.V(lvl.WARNING).Infof("Cleanup %s failed; err=%v", name, err)
		} else {
			glog.V(lvl.INFO).Infof("Cleanup %s after tests", name)
		}
	}
