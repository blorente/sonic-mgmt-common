...
func (d *DB) GetConfig(tables []*TableSpec, opt *GetConfigOptions) (map[TableSpec]Table, error) {
	...
	if d.dbCacheConfig.PerConnection {
		glog.V(lvl.WARNING).Info("GetConfig: Per Connection Cache not supported")
	}
	...
	for scanComplete := false; !scanComplete; {
		...
		for index, redisKey := range redisKeys {
			...
			if result == nil {
				glog.V(lvl.WARNING).Infof("GetConfig: redisKeys[%d]: %v nil", index,
					redisKey)
				continue
			}
			...
			if err != nil {
				glog.V(lvl.WARNING).Infof("GetConfig: redisKeys[%d]: %v err", index, err)
				continue
			}
	...
	if allowWritable {
		...
		if err != nil {
			glog.V(lvl.INFO).Info("GetConfig: applyTxCache failed: ", err)
			return nil, err
		}
	}
