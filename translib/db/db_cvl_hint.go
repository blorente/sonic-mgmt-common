...
func (d *DB) StoreCVLHint(key string, value interface{}) error {
	...
	if glog.V(3) {
		...
		glog.V(lvl.INFO).Info("StoreCVLHint: Begin: key: ", key)
	}
	...
	if (d == nil) || (d.client == nil) {
		...
		glog.V(lvl.INFO).Info("StoreCVLHint: CVL Disabled. Skipping CVL")
	} else {
		if d.cv != nil {
			// TBD Wait for the CVL PR
			...
			glog.V(lvl.INFO).Info("StoreCVLHint: CVL Validation Session not opened.")
...
func (d *DB) clearCVLHint(key string) {
	...
	if (d == nil) || (d.client == nil) {
		glog.V(lvl.WARNING).Infof("clearCVLHint: (d == nil): %t (d.client == nil): %t",
			d == nil, d.client == nil)
	} else if d.cv != nil {
		// TBD Wait for the CVL PR
