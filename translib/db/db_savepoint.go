...
func (d *DB) DeclareSP() error {
	glog.V(lvl.INFO).Infof("DeclareSP: Begin")
	...
	glog.V(lvl.INFO).Infof("DeclareSP: End: %# v", savePoint)
	return nil
}

func (d *DB) ReleaseSP() error {
	glog.V(lvl.INFO).Infof("ReleaseSP: Begin")
	...
	if glog.V(3) {
		...
		glog.V(lvl.INFO).Infof("ReleaseSP: End:")
	}
...
func (d *DB) Rollback2SP() error {
	if glog.V(3) {
		...
		glog.V(lvl.INFO).Infof("Rollback2SP: Begin:")
	}
			...
			if tbl, ok := d.txTsEntryMap[otn]; ok {
				...
				if oEntry.absent {
					...
					if _, okKey := tbl[oRedisKey]; !okKey {
						glog.V(lvl.WARNING).Infof("Rollback2SP: Key missing: %s key: %v",
							otn, oRedisKey)
						hSet = true
					} else {
				...
				glog.V(lvl.WARNING).Infof("Rollback2SP: Table missing: %s val: %v", otn,
					oEntry.value)
			}
		}
	}
	...
	var err error
	if d.cv == nil {
		glog.V(lvl.WARNING).Infof("Rollback2SP: CVL Session Not Opened")
	} else if ret := cvl.ValidationSessClose(d.cv); ret != cvl.CVL_SUCCESS {
		glog.V(lvl.WARNING).Infof("Rollback2SP: Error closing CVL session: ret: %s",
			cvl.GetErrorString(ret))
		err = tlerr.TranslibCVLFailure{Code: int(ret)}
	} else if d.cv, err = d.NewValidationSession(); err != nil {
		glog.V(lvl.WARNING).Infof("Rollback2SP: Error opening CVL session: err: %s", err)
	} else {
		...
		for ix := 0; ix < savePoint.cECDLen; ix++ {
			...
			if (savePoint.cHints != nil) && (savePoint.cHints[ix] != nil) {
				for hKey, hValue := range savePoint.cHints[ix] {
					...
					if cvl.CVL_SUCCESS != hRet {
						glog.V(lvl.WARNING).Infof("Rollback2SP:%d: Hint CVL Failure: %d",
							ix, hRet)
			...
			if cvl.CVL_SUCCESS != ret {
				glog.V(lvl.WARNING).Info("Rollback2SP: CVL Failure: ", ret)
				glog.V(lvl.WARNING).Info("Rollback2SP: ", len(cECD), " ", lenCvlOps)
				err = tlerr.TranslibCVLFailure{Code: int(ret),
					CVLErrorInfo: cei}
				break
			}
	...
	if err != nil {
		glog.V(lvl.WARNING).Info("Rollback2SP: Setting DB in error flag")
		d.err = err
	}
	...
	glog.V(lvl.INFO).Infof("Rollback2SP: End:")
	return err
}
