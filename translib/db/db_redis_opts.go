...
func setGoRedisOpts(optsString string) {
	...
	if optsString != "" {
		glog.V(lvl.INFO).Infof("setGoRedisOpts: optsString: %s", optsString)
		...
		if isDbInstPresent(dbInstName) {
			...
			if len(dbSepStr) > 0 {
				if len(dbOpt.TableNameSeparator) > 0 &&
					...
					glog.V(lvl.WARNING).Infof("TableNameSeparator '%v' in"+
				...
				glog.V(lvl.WARNING).Info("Database Separator not present for the Db name: ",
					dbInstName)
			}
		} else {
			glog.V(lvl.DEBUG).Info("Database instance not present for the Db name: ",
				dbInstName)
		}
	} else {
...
func (config *_DBRedisOptsConfig) reconfigure() error {
	...
	if flag.Parsed() {
		...
		glog.V(lvl.WARNING).Infof("_DBRedisOptsConfig:reconfigure: flags not parsed!")
	}
	...
	if doReconfigure {
		glog.V(lvl.INFO).Infof("_DBRedisOptsConfig:reconfigure: Handling signal.")
		...
		if !reflect.DeepEqual(*config, readDBRedisOptsConfig) {
			glog.V(lvl.INFO).Infof("_DBRedisOptsConfig:reconfigure: Change Detected.")
...
func (config *_DBRedisOptsConfig) parseRedisOptsConfig(optsString string) error {
	...
	glog.V(lvl.INFO).Infof("parseRedisOptsConfig: optsString: %s", optsString)
	...
	for optI, optS := range strings.Split(optsString, ",") {
		glog.V(lvl.INFO).Infof("parseRedisOptsConfig: optI: %d optS: %s", optI, optS)
