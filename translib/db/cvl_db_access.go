...
func (c *cvlDBAccess) Lookup(s ctypes.Search) ctypes.JsonResult {
	...
	if err != nil {
		log.V(lvl.WARNING).Infof("cvlDBAccess: Lookup: error in getTxData: %v", err)
		return strResult{"", err}
	}
...
func (c *cvlDBAccess) Count(s ctypes.Search) ctypes.IntResult {
	...
	if err != nil {
		log.V(lvl.WARNING).Infof("cvlDBAccess: Count: error in getTxData: %v", err)
...
func (pr *pipeKeysResult) update(c *cvlDBAccess) {
	...
	if err != nil {
		log.V(lvl.WARNING).Infof("pipeKeysResult: update: error in Keys pipe query: keys: %v; err: %v", keys, err)
		pr.sRes.err = err
		return
	}
...
func (p *dbAccessPipe) Exec() error {
	...
	if err != nil && err != redis.Nil {
		log.V(lvl.WARNING).Infof("dbAccessPipe: Exec: error in pipeline.Exec; error: %v; "+
