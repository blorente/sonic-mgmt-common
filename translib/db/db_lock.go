...
func (lt *LockStruct) tryLock() error {
	...
	args := []interface{}{"HSETNX", lockTableKey, lt.Name,
		lt.comm + ":" + lt.Id}
	glog.V(lvl.DEBUG).Info("tryLock: RedisCmd: STATE_DB: ", args)
	if reply, err = client.Do(context.Background(), args...).Result(); err == nil {
		if intReply, ok := reply.(int64); !ok {
			...
			lt.locked = true
			glog.V(lvl.DEBUG).Infof("tryLock: Locked: %s:%s", lt.Name, lt.Id)
		}
	}
...
func (lt *LockStruct) unlock() error {
	...
	if reply, err = luaScriptUnlock.Run(context.Background(), client,
		...
		if intReply, ok := reply.(int64); !ok {
			...
			lt.locked = false
			glog.V(lvl.DEBUG).Infof("unlock: Unlocked: %s:%s", lt.Name, lt.Id)
		} else {
			glog.V(lvl.DEBUG).Info("unlock: Already Unlocked")
...
func (lt *LockStruct) dbLockedError(c *redis.Client) error {
	...
	if lt.locked { // locked by self
		...
		glog.V(lvl.WARNING).Infof("dbLockedError: nil db connection; assuming generic lock")
	} else if v, err := c.HGet(context.Background(), lockTableKey, lt.Name).Result(); err != nil {
		glog.V(lvl.WARNING).Infof("dbLockedError: 'HGET %s %s' failed; err=%v", lockTableKey, lt.Name, err)
	} else if parts := strings.SplitN(v, ":", 2); len(parts) != 2 {
		glog.V(lvl.WARNING).Infof("dbLockedError: 'HGET %s %s' returned unknown value %q", lockTableKey, lt.Name, v)
	} else {
		lockId = parts[1]
	}
...
func ConfigDBTryLock(token string) error {
	var err error
	glog.V(lvl.DEBUG).Info("ConfigDBTryLock:")
	if bool(glog.V(4)) || !flag.Parsed() {
	...
	if cdbLock != nil {
		...
		for attempts := 0; attempts < tryLockAttempt; attempts++ {
			...
			glog.V(lvl.DEBUG).Infof("ConfigDBTryLock: Pausing %d ms", tryLockPause)
			time.Sleep(tryLockPause * time.Millisecond)
			glog.V(lvl.DEBUG).Infof("ConfigDBTryLock: Retrying Attempt %d", attempts)
		}
	}
...
func ConfigDBUnlock(token string) error {
	var err error
	glog.V(lvl.DEBUG).Info("ConfigDBUnlock:")
	if bool(glog.V(4)) || !flag.Parsed() {
...
func ConfigDBClearLock() error {
	var err error
	glog.V(lvl.DEBUG).Info("ConfigDBClearLock:")
	...
	if _, ok := err.(tlerr.TranslibDBLock); ok {
		glog.V(lvl.DEBUG).Info("ConfigDBClearLock: Lock Absent")
		err = nil
	}
	return err
}
...
func dumpStack(begin, end int) {
	...
	for idx, line := range stackL {
		...
		glog.V(lvl.DEBUG).Infof("dS:[%d]:[%s]", idx, line)
	}
}
