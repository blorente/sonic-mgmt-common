...
func NewDB(opt Options) (*DB, error) {
	...
	if !d.Opts.IsWriteDisabled {
		if d.dbCacheConfig.PerConnection {
			glog.V(lvl.DEBUG).Info("NewDB: IsWriteDisabled false. Disable Cache")
		}
	...
	if !d.Opts.IsCacheEnabled {
		if d.dbCacheConfig.PerConnection {
			glog.V(lvl.DEBUG).Info("NewDB: IsCacheEnabled false. Disable Cache")
		}
		d.dbCacheConfig.PerConnection = false
	}
	...
	if opt.IsSession && d.dbCacheConfig.PerConnection {
		if d.dbCacheConfig.PerConnection {
			glog.V(lvl.DEBUG).Info("NewDB: IsSession true. Disable Cache")
		}
		d.dbCacheConfig.PerConnection = false
	}
	...
	if len(d.Opts.InitIndicator) == 0 {
		...
		glog.V(lvl.DEBUG).Info("NewDB: RedisCmd: ", d.Name(), ": ", "GET ",
...
func (d *DB) DeleteDB() error {
	...
	if d.txState != txStateNone {
		glog.V(lvl.WARNING).Info("DeleteDB: not txStateNone, txState: ", d.txState)
	}
...
func (d *DB) GetKeys(ts *TableSpec) ([]Key, error) {
	...
	if tNkPat := strings.SplitN(ts.Name, d.Opts.TableNameSeparator,
		...
		pat := Key{Comp: append(strings.Split(tNkPat[1], d.Opts.KeySeparator),
			"*")}
		glog.V(lvl.DEBUG).Infof("GetKeys: Separator in TableSpec %v is Deprecated. "+
...
func (d *DB) DeleteKeys(ts *TableSpec, key Key) error {
	...
	for i := 0; i < len(redisKeys); i++ {
		...
		if e != nil {
			glog.V(lvl.WARNING).Info("DeleteKeys: Deleting: ts: ", ts, " key",
				redisKeys[i], " : ", e)
		}
	}
...
func (d *DB) doCVL(ts *TableSpec, cvlOps []cmn.CVLOperation, key Key, vals []Value) error {
	...
	if d.Opts.DisableCVLCheck {
		glog.V(lvl.DEBUG).Info("doCVL: CVL Disabled. Skipping CVL")
		goto doCVLExit
	}

	// No Transaction case. No CVL.
	if d.txState == txStateNone {
		glog.V(lvl.DEBUG).Info("doCVL: No Transactions. Skipping CVL")
		goto doCVLExit
	}
	...
	if cvl.CVL_SUCCESS != cvlRetCode {
		glog.V(lvl.WARNING).Info("doCVL: CVL Failure: ", cvlRetCode)
		// e = errors.New("CVL Failure: " + string(cvlRetCode))
		e = tlerr.TranslibCVLFailure{Code: int(cvlRetCode),
			CVLErrorInfo: cei}
		glog.V(lvl.DEBUG).Info("doCVL: ", len(d.cvlEditConfigData), len(cvlOps))
...
func (d *DB) doWrite(ts *TableSpec, op _txOp, k Key, val interface{}) error {
	...
	if d.Opts.IsSession && (d.Opts.TxCmdsLim != 0) &&
		...
		glog.V(lvl.INFO).Infof("doWrite: TxCmdsLim exceeded %d >= %d", len(d.txCmds),
			d.Opts.TxCmdsLim)
	...
	if d.txState == txStateNone {
		...
		glog.V(lvl.DEBUG).Info("doWrite: RedisCmd: ", d.Name(), ": ", getOperationName(op), " ", d.key2redis(ts, key), " ", getTableValuesInString(op, val))
...
func (d *DB) setEntry(ts *TableSpec, key Key, value Value, isCreate bool) error {
	...
	if len(value.Field) == 0 {
		if ts.NoDelete {
			glog.V(lvl.DEBUG).Info("setEntry: NoDelete flag is true, skipping deletion of the entry.")
		} else {
			glog.V(lvl.DEBUG).Info("setEntry: Mapping to DeleteEntry()")
...
func (d *DB) RunScript(script *redis.Script, keys []string, args ...interface{}) *redis.Cmd {
	...
	if d.Opts.DBNo == ConfigDB {
		glog.V(lvl.INFO).Info("RunScript: Not supported for ConfigDB")
		return nil
	}
...
func (d *DB) DeleteTable(ts *TableSpec) error {
	...
	for i := 0; i < len(keys); i++ {
		...
		if e != nil {
			glog.V(lvl.WARNING).Info("DeleteTable: DeleteEntry: " + e.Error())
			break
		}
	}
DeleteTableExit:
...
func (d *DB) StartSessTx(w []WatchKeys, tss []*TableSpec) error {
	glog.V(lvl.DEBUG).Info("StartSessTx:")
	return d.startTx(w, tss)
}

func (d *DB) CommitSessTx() error {
	glog.V(lvl.DEBUG).Info("CommitSessTx:")
	return d.commitTx()
}

func (d *DB) AbortSessTx() error {
	glog.V(lvl.DEBUG).Info("AbortSessTx:")
	return d.abortTx()
}
...
func (d *DB) performWatch(w []WatchKeys, tss []*TableSpec) error {
	...
	for i := 0; i < len(w); i++ {
		...
		if e != nil {
			glog.V(lvl.WARNING).Info("performWatch: Keys: " + e.Error())
			if first_e == nil {
				first_e = e
			}
			continue
	...
	if len(args) == 1 {
		glog.V(lvl.WARNING).Info("performWatch: Empty WatchKeys. Skipping WATCH")
		goto SkipWatch
	}
	...
	if e != nil {
		glog.V(lvl.WARNING).Info("performWatch: Do: WATCH ", args, " e: ", e.Error())
		if first_e == nil {
			first_e = e
		}
	}
...
func (d *DB) abortTx() error {
	...
	case txStateWatch:
		if glog.V(lvl.WARNING) {
			glog.Warning("AbortTx: No SET|DEL done, txState: ", d.txState)
		}
	case txStateSet:
		break
	case txStateMultiExec:
	...
	if e != nil {
		glog.V(lvl.WARNING).Info("AbortTx: Do: UNWATCH e: ", e.Error())
	}
...
func (d *DB) markConfigDBUpdated() error {
	...
	if e != nil {
		glog.V(lvl.WARNING).Info("markConfigDBUpdated: Do: SET ",
		...
		if err != nil {
			...
			for i, dbKey := range dbKeys {
				...
				if e != nil {
					...
					glog.V(lvl.WARNING).Infof("GetEntries: %s: error %s; for the key %s",
