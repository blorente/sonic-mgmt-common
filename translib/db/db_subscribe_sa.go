...
func (d *DB) registerSubscribeDB(isSA bool, skeys []*SKey, handler interface{}) {
	...
	glog.V(lvl.DEBUG).Infof("registerSubscribeDB: isSA %t skeys %v", isSA, skeys)
...
func (d *DB) unRegisterSubscribeDB() {
	...
	glog.V(lvl.DEBUG).Info("unRegisterSubscribeDB:")
	...
	if !found {
		glog.V(lvl.DEBUG).Info("unRegisterSubscribeDB: SubscribeDB likely not SA")
		return
	}
