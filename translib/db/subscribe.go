...
func iSubscribeDB(opt Options, skeys []*SKey, handler interface{}) (*DB, error) {
	...
	if !opt.IsWriteDisabled {
		glog.V(lvl.DEBUG).Info("SubscribeDB: Setting IsWriteDisabled")
	...
	glog.V(lvl.DEBUG).Info("SubscribeDB: patterns: ", patterns)
	...
	go func() {
		...
		for msg := range pschan {
			log.V(lvl.DEBUG).Info("SubscribeDB: msg: ", msg)
			curdepth := len(pschan)
			if curdepth > maxchandepth {
				maxchandepth = curdepth
				if maxchandepth > channelDepthWarnThreshold {
					log.V(lvl.WARNING).Infof("Max observed depth for len(pschan): %d", maxchandepth)
				}
			}
		...
		glog.V(lvl.DEBUG).Info("SubscribeDB: SEventClose|Err: ", sEvent)
		if isSA {
			hFuncSA(d, RunningConfigNotif, "", &SKey{}, &Key{}, sEvent)
		} else {
			hFunc(d, &SKey{}, &Key{}, sEvent)
...
func (d *DB) redisChannel2key(ts *TableSpec, redisChannel string) Key {
	...
	glog.V(lvl.WARNING).Info("redisChannel2key: Missing key: redisChannel: ", redisChannel)
